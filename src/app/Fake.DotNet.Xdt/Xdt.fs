...
module Xdt =
    ...
    type internal FakeXmlTransformationLogger() =
        interface IXmlTransformationLogger with
            ...
            member x.LogErrorFromException(ex) = Trace.traceException ex
    ...
    let transformFile (inXmlFile: string) (transformFile: string) (outXmlFile: string) =
        ...
        if not <| File.Exists transformFile then
            Trace.traceErrorfn "XML Document Transform file %s does not exist." transformFile

        use xdtStream =
            new FileStream(Path.GetFullPath transformFile, FileMode.Open, FileAccess.Read)

        let fakeLogger = FakeXmlTransformationLogger()
        use xdt = new XmlTransformation(xdtStream, fakeLogger)
        let xml = XmlDocument()
        xml.Load(Path.GetFullPath inXmlFile)

        if not <| xdt.Apply(xml) then
            InvalidOperationException(sprintf "Unable to transform %s with %s." inXmlFile transformFile)
            |> raise
