...
let rec private writeChunkToReqStream (chunk:byte[]) (reqStrm:Stream) (br:BinaryReader) =
    if chunk.Length <> 0 
    then reqStrm.Write(chunk, 0, chunk.Length)
         writeChunkToReqStream (br.ReadBytes 1024) reqStrm |> ignore

let inline private getSubstring (fromPos:int) (str:string) (toPos:int) =
     str.Substring(fromPos, toPos)

let inline private lastSlashPos (str:string) =
    str.LastIndexOf(@"\")+1
...
let inline private dirNameIsValid (dirName:string) =
    let invalidChars = ["<"; ">"; ":"; "\""; "/"; "\\"; "|"; "?"; "*"]
    let invalidNames = ["CON"; "PRN"; "AUX"; "NUL"; "COM1"; "COM2"; "COM3"; "COM4"; "COM5"; "COM6"; "COM7"; "COM8"; "COM9"; "LPT1"; "LPT2"; "LPT3"; "LPT4"; "LPT5"; "LPT6"; "LPT7"; "LPT8"; "LPT9"]
...
let rec uploadAFolder server user pwd (srcPath:string) (rootDir:string) = 
    ...
    if dirInfo.Exists && dirNameIsValid rootDir
    then dirInfo.GetFileSystemInfos () |> Seq.iter (fun fsi -> upload server user pwd fsi rootDir)

and private upload server user pwd (fsi:FileSystemInfo) (rootDir:string) =
    match fsi.GetType().ToString() with
        | "System.IO.DirectoryInfo" -> createAFolder server user pwd rootDir 
