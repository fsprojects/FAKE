...
[<Obsolete "Use the TargetType in FscHelper.Alt instead">]
type FscTarget = 
    | Exe
    | Winexe
    | Library
...
[<Obsolete "Use PlatformType in FscHelper.Alt instead">]
type FscPlatform = 
    | X86
    | Itanium
    | X64
...
[<Obsolete "Use FscParam in FscHelper.Alt instead">]
type FscParams = 
...
[<Obsolete "Use fscList in FscHelper.Alt instead">]
...
///                                FscTarget = ...
///                                ... })
[<Obsolete "Use fsc in FscHelper.Alt instead">]
...
module Alt =
    ...
    /// given (i.e. the second argument is an empty list), by default tries
    /// to behave the same way as would the command-line 'fsc.exe' tool.
    let fsc_list (srcFiles : string list) (opts : string list) : int = 
        let scs = SimpleSourceCodeServices()
    ...
    ///             Debug false 
    ///        ]
    let fsc (fscParams : FscParam list) (inputFiles : string list) : int = 
        let inputFiles = inputFiles |> Seq.toList
        let taskDesc = inputFiles |> separated ", "
    ...
    ///             Debug false 
    ///        ]
    let Fsc (fscParams : FscParam list) (inputFiles : string list) : unit = 
        let res = fsc fscParams inputFiles
        if res <> 0 then raise <| BuildException("Fsc: compile failed with exit code", [ string res ])
