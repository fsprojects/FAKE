...
let mutable internal files = Dictionary()

let fileName file =
    match files.TryGetValue file with
    | true,n -> files.[file] <- n+1; file + n.ToString()
    | _ -> files.[file] <- 1; file
...
let wixFile (fileInfo:FileInfo) =
    sprintf "<File Id=\"%s\" Name=\"%s\" Source=\"%s\" />" (fileName fileInfo.Name) fileInfo.Name fileInfo.FullName
