...
module TeamCity =
    ...
    let block name description =
        ...
        { new IDisposable with
            member __.Dispose() = TeamCityWriter.sendCloseBlock name }

    /// Sends an error to TeamCity
    let sendTeamCityError error =
        TeamCityWriter.sendBuildStatus "FAILURE" error
    ...
    module internal Import =
        ...
        let sendNUnit path =
            TeamCityWriter.sendImportData "nunit" path

        /// Sends an FXCop results filename to TeamCity
        let sendFXCop path =
            TeamCityWriter.sendImportData "FxCop" path

        /// Sends an JUnit Ant task results filename to TeamCity
        let sendJUnit path =
            TeamCityWriter.sendImportData "junit" path

        /// Sends an Maven Surefire results filename to TeamCity
        let sendSurefire path =
            TeamCityWriter.sendImportData "surefire" path

        /// Sends an MSTest results filename to TeamCity
        let sendMSTest path =
            TeamCityWriter.sendImportData "mstest" path

        /// Sends an Google Test results filename to TeamCity
        let sendGTest path =
            TeamCityWriter.sendImportData "gtest" path

        /// Sends an Checkstyle results filename to TeamCity
        let sendCheckstyle path =
            TeamCityWriter.sendImportData "checkstyle" path

        /// Sends an FindBugs results filename to TeamCity
        let sendFindBugs path =
            TeamCityWriter.sendImportData "findBugs" path

        /// Sends an JSLint results filename to TeamCity
        let sendJSLint path =
            TeamCityWriter.sendImportData "jslint" path

        /// Sends an ReSharper inspectCode.exe results filename to TeamCity
        let sendReSharperInspectCode path =
            TeamCityWriter.sendImportData "ReSharperInspectCode" path

        /// Sends an PMD inspections results filename to TeamCity
        let sendPmd path =
            TeamCityWriter.sendImportData "pmd" path

        /// Sends an PMD Copy/Paste Detector results filename to TeamCity
        let sendPmdCpd path =
            TeamCityWriter.sendImportData "pmdCpd" path

        /// Sends an ReSharper <c>dupfinder.exe</c> results filename to TeamCity
        let sendDotNetDupFinder path =
            TeamCityWriter.sendImportData "DotNetDupFinder" path
        ...
        let sendDotNetCoverageForTool path (tool: DotNetCoverageTool) =
            TeamCityWriter.sendImportDataWithTool "dotNetCoverage" tool.TeamCityName path

    /// Sends the full path to the dotCover home folder to override the bundled dotCover to TeamCity
    let internal sendTeamCityDotCoverHome =
        TeamCityWriter.sendDotNetCoverage "dotcover_home"

    /// Sends the full path to NCover installation folder to TeamCity
    let internal sendTeamCityNCover3Home =
        TeamCityWriter.sendDotNetCoverage "ncover3_home"

    /// Sends arguments for the NCover report generator to TeamCity
    let internal sendTeamCityNCover3ReporterArgs =
        TeamCityWriter.sendDotNetCoverage "ncover3_reporter_args"

    /// Sends the path to NCoverExplorer to TeamCity
    let internal sendTeamCityNCoverExplorerTool =
        TeamCityWriter.sendDotNetCoverage "ncover_explorer_tool"

    /// Sends additional arguments for NCover 1.x to TeamCity
    let internal sendTeamCityNCoverExplorerToolArgs =
        TeamCityWriter.sendDotNetCoverage "ncover_explorer_tool_args"

    /// Sends the value for NCover /report: argument to TeamCity
    let internal sendTeamCityNCoverReportType: int -> unit =
        string >> TeamCityWriter.sendDotNetCoverage "ncover_explorer_report_type"

    /// Sends the value for NCover  /sort: argument to TeamCity
    let internal sendTeamCityNCoverReportOrder: int -> unit =
        string >> TeamCityWriter.sendDotNetCoverage "ncover_explorer_report_order"
    ...
    let internal startTestCase testCaseName =
        TeamCityWriter.sendTestStarted testCaseName
    ...
    let internal reportTestOutput name output =
        TeamCityWriter.sendTestStdOut name output

    /// Report Standard-Error for a given test-case
    let internal reportTestError name output =
        TeamCityWriter.sendTestStdError name output
    ...
    let internal finishTestSuite testSuiteName =
        TeamCityWriter.sendTestSuiteFinished testSuiteName

    /// Starts the test suite.
    let internal startTestSuite testSuiteName =
        TeamCityWriter.sendTestSuiteStarted testSuiteName

    /// Reports the progress.
    let reportProgress message =
        TeamCityWriter.sendProgressMessage message

    /// Reports the progress start.
    let reportProgressStart message =
        TeamCityWriter.sendProgressStart message

    /// Reports the progress end.
    let reportProgressFinish message =
        TeamCityWriter.sendProgressFinish message

    /// Reports the build status.
    let reportBuildStatus status message =
        TeamCityWriter.sendBuildStatus status message
    ...
    let internal publishNamedArtifact name path =
        TeamCityWriter.sendPublishNamedArtifact name path

    /// Sets the TeamCity build number.
    let internal setBuildNumber buildNumber =
        TeamCityWriter.sendBuildNumber buildNumber

    /// Reports a build statistic.
    let setBuildStatistic key value =
        TeamCityWriter.sendBuildStatistic key value

    /// Reports a parameter value
    let setParameter name value =
        TeamCityWriter.sendSetParameter name value

    /// Reports a failed test
    let internal testFailed name message details =
        TeamCityWriter.sendTestFailed name message details

    /// Reports a failed comparison
    let internal comparisonFailure name message details expected actual =
        TeamCityWriter.sendComparisonFailed name message details expected actual

    /// Sends a warning message.
    let internal warning message =
        TeamCityWriter.sendMessage "WARNING" message

    /// Sends an error message.
    let internal error message =
        TeamCityWriter.sendMessage "ERROR" message
    ...
    type BuildParameters =
        ...
        static member System = TeamCityBuildParameters.getAllSystem ()

        /// Get all configuration build parameters
        static member Configuration = TeamCityBuildParameters.getAllConfiguration ()

        /// Get all runner build parameters
        static member Runner = TeamCityBuildParameters.getAllRunner ()

        /// Get all build parameters
        ///
        /// System ones are prefixed with 'system.', runner ones with 'runner.' and environment variables with 'env.'
        static member All = TeamCityBuildParameters.getAll ()
    ...
    type Environment =
        ...
        static member Branch = BuildParameters.Configuration |> Map.tryFind "vcsroot.branch"
        ...
        static member CheckoutDirectory =
            BuildParameters.System |> Map.tryFind "teamcity.build.checkoutDir"
        ...
        /// the documentation</a> for more information
        static member ChangedFiles = ChangedFiles.cache.Value

        /// Name of recently failing tests
        /// See <a href="https://confluence.jetbrains.com/display/TCD18/Risk+Tests+Reordering+in+Custom+Test+Runner">
        /// the documentation</a> for more information
        static member RecentlyFailedTests = RecentlyFailedTests.cache.Value
    ...
    /// [omit]
    let defaultTraceListener = TeamCityTraceListener() :> ITraceListener

    /// [omit]
    let detect () = BuildServer.buildServer = TeamCity

    /// [omit]
    let install (force: bool) =
        if not (detect ()) then
            failwithf "Cannot run 'install()' on a non-TeamCity environment"
