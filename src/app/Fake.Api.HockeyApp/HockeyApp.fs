...
module HockeyApp =
    ...
    let private validateParams param:HockeyAppUploadParams =
        ...
        if not (String.IsNullOrEmpty param.Dsym) then
            if not (param.Dsym.EndsWith(".dsym.zip", StringComparison.OrdinalIgnoreCase) 
            || param.Dsym.Equals("mapping.txt", StringComparison.OrdinalIgnoreCase)) then
                failwith "DSYM files should only be: IOS: *.dsym.zip  Android: mapping.txt"

            if param.File.EndsWith(".ipa") 
            && not (param.Dsym.EndsWith(".dsym.zip", StringComparison.OrdinalIgnoreCase)) then
                failwith "DSYM for an .ipa file can only only be: *.dsym.zip"

            if param.File.EndsWith(".apk") 
            && not (param.Dsym.Equals("mapping.txt", StringComparison.OrdinalIgnoreCase)) then
    ...
    let private processHockeyAppCmd<'TParam, 'TResponse when 'TParam :> IBaseHockeyAppParams> defaults
        ...
        |> fun response ->
            ...
            match response.ExitCode with
                | 0 ->
                    match Int32.TryParse (response.Messages.[response.Messages.Length - 1].Trim()) with
    ...
    let uploadApp setParams = processHockeyAppCmd<HockeyAppUploadParams, HockeyResponse> HockeyAppUploadDefaults setParams validateParams toCurlArgs
    ...
    let createAppVersion setParams = processHockeyAppCmd<HockeyAppVersionParams, HockeyVersionResponse> HockeyAppVersionDefaults setParams validateVersionParams toVersionCurlArgs
