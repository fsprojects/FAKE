...
let (->!) (argv': string) val' (doc': Docopt) =
    ...
    let msg, res =
        try
            let _ = doc'.Parse(argv).AsList() in (box "<NO EXN>", false)
        with e ->
            (box e, e.GetType() = val')
...
printfn
    "\n>>> %i Docopt calls in %A\nless than %fms per call"
    count
    stopwatch.Elapsed
    (float stopwatch.ElapsedMilliseconds / float count)
