...
exception DocoptException of string with
    override x.Message = sprintf "%s" x.Data0
...
exception UsageException of string with
    override x.Message = sprintf "%s" x.Data0
...
module private Helpers =
    ...
    let improveErrorText (lnNr: int64) (colNr: int64) (arg: string) (oldText: string) =
        oldText.Replace(sprintf "argv: Ln: %d Col: %d" lnNr colNr, sprintf "Argument %d ('%s')" (colNr + 1L) arg)

    let unexpectedShort = (+) "short option -" >> unexpected
    let unexpectedLong = (+) "long option --" >> unexpected
    let expectedArg = (+) "argument " >> expected
    let unexpectedArg = (+) "argument " >> unexpected
...
type ArgumentStreamState<'TUserState> =
    { Pos: ArgumentStreamPosition
      StateTag: int64
      UserState: 'TUserState }

    interface IArgumentStreamState<'TUserState>

type ArgumentStream<'TUserState> private (argv: string array, initState: 'TUserState) =
    let mutable pos =
        if argv.Length > 0 && ArgumentArray.isShortArgument argv.[0] then
            ShortArgumentPartialPos(0, 1)
        else
            ArgumentPos 0

    let mutable stateTag = 0L
    let mutable state = initState
    let markChange () = stateTag <- stateTag + 1L
...
module ArgumentStream =
    ...
    type MappingStreamState<'u, 't, 'tinner> =
        ...
        interface IArgumentStreamState<'t>
...
module ArgParser =
    ...
    let pzero: ArgumentParser<_, _> =
        fun stream -> Reply(Error, FParsec.Error.NoErrorMessages)
    ...
    let choiceBest (ps: seq<ArgumentParser<'u, 'a>>) : ArgumentParser<_, _> =
        fun (stream: IArgumentStream<_>) ->
            ...
            let maxArgLength =
                (Seq.append stream.Argv [ "" ] |> Seq.maxBy (fun (arg: string) -> arg.Length))
                    .Length
            ...
            let mutable (bestStream, bestResult) =
                ...
                |> Seq.maxBy (fun (stream, reply) ->
                    if reply.Status <> Ok then
                        -stream.Argv.Length - 1 + stream.Position.ArgIndex,
                        -maxArgLength - 1 + stream.Position.InnerIndex
                    else
                        stream.Position.ArgIndex, stream.Position.InnerIndex)
            ...
            let reply =
                if bestResult.Status <> Ok then
                    let errors =
                        results
                        |> Seq.fold
                            (fun (errors: ErrorMessageList) (_, reply) -> mergeErrors errors reply.Error)
                            NoErrorMessages
            ...
            while stream.Position <> bestStream.Position do
                stream.Read() |> ignore
    ...
    type Inline =
        ...
        static member inline Many
            ...
            fun stream ->
                ...
                let firstElementParser =
                    match firstElementParser with
                    | Some p -> p
                    | _ -> elementParser
        ...
        fun (stream: IArgumentStream<_>) ->
            ...
            while reply.Status = ReplyStatus.Ok && availableParsers.Length > 0 do
                ...
                availableParsers <-
                    ...
                    |> List.filter (fun (i, (allowMultiple, _)) ->
                        allowMultiple
                        || not (List.exists (fun applied -> applied = i) newStream.UserState.AppliedParsers))
                    |> List.map snd

            if reply.Status = Error && not allowMissing then
                Reply(reply.Status, reply.Error)
            else if not allowEmpty && results = [] then
                Reply(reply.Status, reply.Error)
            else
                Reply(results)
    ...
    let pcmd cmd =
        let chooseCmd arg = if arg = cmd then Some cmd else None
        chooseParserFull' (sprintf "Command '%s'" cmd) chooseCmd

    let parg argName =
        chooseParserFull (sprintf "Argument for  '%s'" argName) id
    ...
    let multipleSaveInMap f =
        updateUserState (fun item map -> f item |> Seq.fold (fun map (key, newItem) -> updateMap key newItem map) map)

    let mergeMap m1 m2 =
        Map.fold (fun s k v -> Map.add k v s) m1 m2

    let mergeMaps maps = Seq.fold mergeMap Map.empty maps


    let pLongFlag (flag: SafeOption) =
        if not flag.IsLong then
            failwithf "Cannot parse empty short flag %O" flag

        let keys =
            [ if flag.IsShort then
                  yield flag.FullShort
              if flag.IsLong then
                  yield flag.FullLong ]
        ...
        if flag.HasArgument then
            ...
            chooseParserFull (sprintf "Flag '%s='" flag.FullLong) chooseCmd
            <|> (single >>. parg flag.FullLong)
            >>= saveInMapM keys (DocoptResult.Argument)
        else
            single >>= saveInMapM keys (fun _ -> DocoptResult.Flag)


    let pShortFlag (flag: SafeOption) =
        if not flag.IsShort then
            failwithf "Cannot parse empty short flag %O" flag
    ...
    let pOption includeShort (o': SafeOption) =
        let longArg = if o'.IsLong then pLongFlag o' else pzero
    ...
    let pOptions allowMissing (flags: SafeOptions) =
        let optionParsers =
            flags
            |> Seq.map (fun flag -> flag.AllowMultiple, pOption true flag)
            |> Seq.toList
    ...
    let rec getParser (ast: UsageAst) : ArgumentParser<_, _> =
        let p =
            ...
            | UsageAst.Ell ast' ->
                // One or more
                many1 (getParser ast') >>= updateUserState (fun _ state -> state)
            | UsageAst.Sdh -> pcmd "-" >>= saveInMap "-" (fun _ -> DocoptResult.Flag)
        //(debug (fun _ stream ->
...
type UsageParser(usageStrings': string array, sections: (string * SafeOptions) list) =
    ...
    let pupperArg =
        ...
        identifier (
            IdentifierOptions(isAsciiIdStart = start, isAsciiIdContinue = cont, label = "UPPER-CASE identifier")
        )
    ...
    let parg =
        let filterArg arg' (last': UsageAstCell) =
            if obj.ReferenceEquals(null, last') then
    ...
    let psdh =
        skipString "[-]"
        >>= updateUserState (fun _ _ ->
            UsageAstBuilder.Sqb(UsageAstBuilder.Sdh |> UsageAstBuilder.ToCell)
            |> UsageAstBuilder.ToCell)

    let psop =
        let filterSops (sops': string) (last': UsageAstCell) =
            ...
            while (i <- i + 1
                ...
                | Some opt ->
                    (if opt.HasArgument && i + 1 < sops'.Length then
                         i <- sops'.Length)

                    sops.Add(opt)

            if sops.Count = 0 then
                UsageAstBuilder.Eps
            else
                match last'.Content with
        ...
        skipChar '-' >>. many1SatisfyL (isLetterOrDigit) "Short option(s)"
    ...
    let plop =
        ...
        skipString "--" >>. manySatisfy (fun c' -> Char.IsLetterOrDigit(c') || c' = '-')
        .>>. opt (skipChar '=' >>. (plowerArg <|> pupperArg))
        >>= updateUserState filterLopt

    let psqb =
        between (skipChar '[' >>. spaces) (skipChar ']') opp.ExpressionParser
    ...
    let pcmd =
        many1Satisfy (fun c' -> isLetter (c') || isDigit (c') || c' = '-')
        >>= updateUserState (fun cmd' _ -> UsageAstBuilder.Cmd(cmd') |> UsageAstCell.FromBuilder)

    let panoParsers = sections |> List.map pano

    let term =
        choice (Seq.append panoParsers [| psdh; plop; psop; psqb; preq; parg; pcmd |])

    let pxor =
        let afterStringParser =
            spaces
            .>> updateUserState (fun _ _ -> UsageAstBuilder.XorEmpty |> UsageAstCell.FromBuilder) () in
    ...
    let pell =
        let afterStringParser =
            ...
            .>> updateUserState
                    (fun _ _ ->
                        UsageAstBuilder.Ell(UsageAstBuilder.Eps |> UsageAstCell.FromBuilder)
        ...
        let makeEll (ast': UsageAstCell) =
            ...
            | Some(UsageAstBuilder.Seq seq) ->
                ...
                cell.Content <-
                    ...
                    | Some c -> Some(UsageAstBuilder.Ell(UsageAstCell.FromBuilder c))
                    | None -> Some <| UsageAstBuilder.Ell(UsageAstBuilder.Eps |> UsageAstCell.FromBuilder)

                ast'
            | _ -> UsageAstBuilder.Ell(ast') |> UsageAstCell.FromBuilder in

        PostfixOperator("...", afterStringParser, 20, false, makeEll)
    ...
    let _ =
        opp.TermParser <-
            ...
            >>= updateUserState (fun ast' _ ->
                match
                    ast'
                    |> List.filter (fun ast' -> ast'.Content.IsSome && ast'.Content.Value.UsageTag <> Tag.Eps)
                with
    ...
    do
        if
            usageStrings'.Length = 0
            || usageStrings' |> Seq.forall (String.IsNullOrWhiteSpace)
        then
            failwithf "Not given any usage-formats"
    ...
    member __.ParseCommandLine(argv) =
        ...
        let parseError =
            ParserError(Position("argv", argIdx, 0L, argIdx), state.UserState, reply.Error)
        ...
        let errorText =
            ...
            sw.ToString()
            |> Helpers.improveErrorText
                0L
                argIdx
                (if argIdx >= 0L && int argIdx < argv.Length then
                     argv.[int argIdx]
                 else
                     "<>")
        ...
        | _, _, true ->
            raise
            <| DocoptException(sprintf "errors %s: %s" (printReplyStatus reply.Status) errorText)
        | _, [||], false ->
            let unparsed = argv.[state.Position.ArgIndex .. argv.Length - 1]

            raise
            <| DocoptException(sprintf "'[|%s|]' could not be parsed" (System.String.Join(";", unparsed :> _ seq)))
        | _ ->
            let unparsed = argv.[state.Position.ArgIndex .. argv.Length - 1]
            ...
            <| DocoptException(
                sprintf
                    "errors: %s, ('[|%s|]' could not be parsed)"
                    errorText
                    (System.String.Join(";", unparsed :> _ seq))
            )
