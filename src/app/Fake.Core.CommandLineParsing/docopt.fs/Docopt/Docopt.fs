...
module DocHelper =
    ...
    let internal (|Usage|Options|Other|Newline|) (line': string) =
        if String.IsNullOrEmpty(line') then
            ...
            let idx = line'.IndexOf("usage:", OrdinalIgnoreCase) in

            if idx <> -1 then
                Usage(line'.Substring(idx + 6))
            else
                let idx = line'.IndexOf("options", OrdinalIgnoreCase)
                let idxCol = line'.IndexOf(":", OrdinalIgnoreCase)

                if idx <> -1 && idxCol <> -1 then
    ...
    type internal OptionBuilder =
        ...
        member x.Build() =
            { OptionSection.Title = x.Title
              OptionSection.Lines = x.Lines |> List.rev }
    ...
    let cut (doc': string) =
        ...
        |> fun (ustr', ostrs', _) ->
            ...
            let ostrsArray = ostrs' |> List.map OptionBuilder.Build |> List.toArray
            ...
            let groupedResults =
                ...
                |> Array.map (fun (title, group) ->
                    { OptionSection.Title = title
                      OptionSection.Lines = group |> Seq.map (fun item -> item.Lines) |> Seq.concat |> Seq.toList })
                    ...
                    (fun map opt ->
                        let addKey def key =
                            if not (String.IsNullOrEmpty key) then // opt.FullShort else opt.FullLong
                                ...
                                | Some defVal, None -> Map.add key (DocoptResult.Argument defVal)
                                | _ -> id
                            else
                                id
