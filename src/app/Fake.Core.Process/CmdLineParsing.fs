    ...
    let windowsCommandLineToArgv (arguments: string) =
        if arguments.Contains "\"\"\"" then
            invalidArg
                "arguments"
                (sprintf
                    "tripple quotes are not allowed in the command line ('%s') as they behave different across programs, see https://github.com/vbfox/FoxSharp/issues/1 to escape a quote use backslash and the rules from https://docs.microsoft.com/en-US/cpp/cpp/parsing-cpp-command-line-arguments?view=vs-2017."
                    arguments)
        ...
        while i < arguments.Length do
            ...
            if backslashCount > 0 then
                ...
                | _ -> currentArgument.Append(c) |> ignore
...
module Args =
    ...
    let toWindowsCommandLine args =
        CmdLineParsing.windowsArgvToCommandLine true args
    ...
    let fromWindowsCommandLine cmd =
        CmdLineParsing.windowsCommandLineToArgv cmd
...
type Arguments =
    ...
    member x.ToStartInfo =
        ...
        | Some orig -> orig
        | None -> CmdLineParsing.toProcessStartInfo x.Args
...
module Arguments =
    ...
    let withPrefix (s: string seq) (a: Arguments) = Arguments.OfArgs(Seq.append s a.Args)
    /// Append all arguments after the current arguments
    let append s (a: Arguments) = Arguments.OfArgs(Seq.append a.Args s)
    ...
    let appendRaw s (a: Arguments) =
        ...
        { Args = Array.append a.Args [| s |]
          Original = Some newCmd }
    ...
    let appendRawEscaped (argPrefix: string) paramValue (a: Arguments) =
        ...
        a
        |> appendRaw (sprintf "%s%s" argPrefix (CmdLineParsing.windowsArgvToCommandLine false [ paramValue ]))
    ...
    let appendRawEscapedIf b (argPrefix: string) (paramValue: string) (a: Arguments) =
        if b then a |> appendRawEscaped argPrefix paramValue else a
    ...
    let appendIf value paramName (a: Arguments) =
        if value then a |> append [ paramName ] else a
    ...
    let appendNotEmpty paramName paramValue (a: Arguments) =
        if String.isNullOrEmpty paramValue then
            ...
            a |> append [ paramName; paramValue ]

    /// Convert the arguments instance to a string list
    let toList (a: Arguments) = a.Args |> Array.toList

    /// Convert the arguments instance to a string array
    let toArray (a: Arguments) = a.Args |> Array.toList |> Seq.toArray
