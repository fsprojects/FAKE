...
module IMap =
    ...
    let inline remove k (m: IMap<_, _>) : IMap<_, _> = m.Remove(k)

    let inline iter f (m: IMap<_, _>) =
        for kv in m do
            f kv.Key kv.Value

    let inline add k v (m: IMap<_, _>) : IMap<_, _> = m.SetItem(k, v)
...
module EnvMap =
    let empty =
        if Environment.isWindows then
            ImmutableDictionary.Empty.WithComparers(StringComparer.OrdinalIgnoreCase) :> EnvMap
        else
            IMap.empty
    ...
    let replace l (e: EnvMap) : EnvMap = e.SetItems(l)
    //|> IMap.add defaultEnvVar defaultEnvVar

    let ofMap l : EnvMap = create () |> replace l
...
type DataRef<'T> =
    internal
        { mutable value: 'T option
          mutable onSet: 'T -> unit }

    static member Empty = { value = None; onSet = ignore }: DataRef<'T>

    static member Map f (inner: DataRef<'T>) =
        let newCell =
            { value = inner.value |> Option.map f
              onSet = fun _ -> invalidOp "cannot set this ref cell" }
    ...
    member x.Value =
        match x.value with
        | Some s -> s
        | None -> invalidOp "Cannot retrieve data cell before it has been set!"
...
type internal StreamSpecs =
    ...
    member x.SetStartInfo(p: System.Diagnostics.ProcessStartInfo) =
        ...
        | Inherit -> p.RedirectStandardInput <- false
        | UseStream _
        | CreatePipe _ -> p.RedirectStandardInput <- true

        match x.StandardOutput with
        | Inherit -> p.RedirectStandardOutput <- false
        | UseStream _
        | CreatePipe _ -> p.RedirectStandardOutput <- true

        match x.StandardError with
        | Inherit -> p.RedirectStandardError <- false
        | UseStream _
        | CreatePipe _ -> p.RedirectStandardError <- true
...
type internal RawCreateProcess =
    ...
    member internal x.ToStartInfo =
        ...
        | Some work -> p.WorkingDirectory <- work
        | None -> ()
                    ...
                    let start () =
                        if not <| isStarted then
                                ...
                                async {
                                    ...
                                    | UseStream (shouldClose, stream) ->
                                        if isInputStream then
                                            ...
                                            do!
                                                processStream.CopyToAsync(stream, 81920, tok.Token)
                                                |> Async.AwaitTaskWithoutAggregate

                                        return if shouldClose then stream else Stream.Null
                                    | CreatePipe r ->
                            ...
                            if p.RedirectStandardInput then
                                redirectStdInTask <-
                                    handleStream
                                        c.Streams.StandardInput
                                        streamSpec.StandardInput
                                        toolProcess.StandardInput.BaseStream
                                        true
                            ...
                            if p.RedirectStandardOutput then
                                readOutputTask <-
                                    handleStream
                                        c.Streams.StandardOutput
                                        streamSpec.StandardOutput
                                        toolProcess.StandardOutput.BaseStream
                                        false
                                    // Immediate makes sure we set the ref cell before we return...
                                    |> fun a -> Async.StartImmediateAsTask(a, cancellationToken = tok.Token)
                            ...
                            if p.RedirectStandardError then
                                readErrorTask <-
                                    handleStream
                                        c.Streams.StandardError
                                        streamSpec.StandardError
                                        toolProcess.StandardError.BaseStream
                                        false
                                    // Immediate makes sure we set the ref cell before we return...
                                    |> fun a -> Async.StartImmediateAsTask(a, cancellationToken = tok.Token)
                        ...
                        async {
                            ...
                            let all =
                                System.Threading.Tasks.Task.WhenAll(
                                    [ readErrorTask; readOutputTask; redirectStdInTask ]
                                )
                            ...
                            while not allFinished && retries > 0 do
                                ...
                                if not ok && retries < 6 then
                                    Trace.traceFAKE
                                        "At least one redirection task did not finish: \nReadErrorTask: %O, ReadOutputTask: %O, RedirectStdInTask: %O"
                                        readErrorTask.Status
                                        readOutputTask.Status
                                        redirectStdInTask.Status
                            ...
                            if
                                allFinished
                                || Environment.GetEnvironmentVariable("FAKE_DEBUG_PROCESS_HANG") = "true"
                            then
                                if
                                    not allFinished
                                    && Environment.GetEnvironmentVariable("FAKE_ATTACH_DEBUGGER") = "true"
                                then
                                ...
                                if
                                    not allFinished
                                    && Environment.GetEnvironmentVariable("FAKE_FAIL_PROCESS_HANG") = "true"
                                then
                                ...
                                let! streams =
                                    all.ContinueWith(
                                        System.Func<System.Threading.Tasks.Task<Stream[]>, Stream[]>(fun t ->
                                ...
                                let msg =
                                    "We encountered https://github.com/fsharp/FAKE/issues/2401, please help to resolve this issue! You can set 'FAKE_IGNORE_PROCESS_HANG' to true to ignore this error. But please consider sending a full process dump or volunteer with debugging."
                                ...
                                if Environment.GetEnvironmentVariable("FAKE_IGNORE_PROCESS_HANG") <> "true" then
                                    ...
                                    async {
                                        try
                                            let! streams =
                                                all.ContinueWith(
                                                    System.Func<System.Threading.Tasks.Task<Stream[]>, Stream[]>
                                                        (fun t -> t.GetAwaiter().GetResult())
                                                )
                                                |> Async.AwaitTaskWithoutAggregate

                                            Trace.traceFAKE
                                                "The hanging redirect task has finished eventually! Disposing streams."
