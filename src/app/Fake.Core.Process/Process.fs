...
module private ProcStartInfoData =
    ...
    let checkMap (map:Map<string,string>) =
        if Environment.isWindows then
            let hs = System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase)
            for kv in map do
        ...
        | Some path ->
            let result =
                try
                    ...
                    | true -> ProcessResult.New 0 [{ ConsoleMessage.IsError = true; ConsoleMessage.Message = String.toLines errors;ConsoleMessage.Timestamp = DateTimeOffset.Now }]
                    | false -> ProcessResult.New 0 [{ ConsoleMessage.IsError = false; ConsoleMessage.Message = String.toLines results; ConsoleMessage.Timestamp = DateTimeOffset.Now }]
...
type Shell private() = 
    ...
    static member AsyncExec(cmd, ?args, ?dir) =
        ...
        let commandLine = internalArgs.CommandLine + " " + Process.formatArgs internalArgs.Args
