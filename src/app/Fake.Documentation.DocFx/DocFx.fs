...
module DocFx =
    ...
    let private docFxPath =
        ...
        let toolPath =
            ProcessUtils.tryFindLocalTool "TOOL" docFxExe [ Directory.GetCurrentDirectory() ]
    ...
    let exec setParams command args =
        ...
        let processResult =
            CreateProcess.fromRawCommandLine p.DocFxPath commandArgs
            |> CreateProcess.withWorkingDirectory p.WorkingDirectory
    ...
    let private stringify value = sprintf "%A" value
    ...
    let private stringifyParams parameters =
        ...
        let args = parameters |> List.map stringify |> Arguments.OfArgs
    ...
    type BuildParams =
        ...
        member inline x.WithLogParams f = { x with LogParams = f x.LogParams }
    ...
    let private serializeExportTemplateParams p =
        [ ("all", stringify p.All); ("output", p.OutputFolder) ]
        |> stringifyParams
    ...
    let private serializeServeParams p =
        [ ("hostname", p.Host); ("port", stringifyOption p.Port) ]
        |> stringifyParams
