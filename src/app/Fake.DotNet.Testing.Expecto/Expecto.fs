...
module Expecto =
    ...
    type Params =
        ...
        static member DefaultParams =
            { Debug = false
              ...
              Run = []
              ListTests = false
              // Summary = true somehow breaks Expecto TeamCity printer
              Summary = false
              SummaryLocation = false
              CustomArgs = []
              WorkingDirectory = "" }

    type private RunMode =
        | Direct
        | DotNetCli
    ...
    let private runAssembly expectoParams testAssembly =
        ...
        let workingDir =
            if String.isNotNullOrEmpty expectoParams.WorkingDirectory then
                expectoParams.WorkingDirectory
            else
                Fake.IO.Path.getDirectory testAssembly

        let fileName, argsString =
            match runMode with
            | DotNetCli -> "dotnet", sprintf "\"%s\" %O" testAssembly expectoParams
            | Direct -> testAssembly, string expectoParams

        let processResult =
            CreateProcess.fromRawCommandLine fileName argsString
    ...
    let run (setParams: Params -> Params) (assemblies: string seq) =
        ...
        | failedAssemblies ->
            ...
            |> List.map (fun (testAssembly, exitCode) ->
                sprintf
                    "Expecto test of assembly '%s' failed. Process finished with exit code %d."
                    testAssembly
                    exitCode)
