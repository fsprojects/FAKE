...
module DirectoryInfo =
    ...
    let rec isSubfolderOf (dir2: DirectoryInfo) (dir1: DirectoryInfo) =
        if Path.normalizeFileName dir1.FullName = Path.normalizeFileName dir2.FullName then
            true
        else if isNull dir1.Parent then
            false
        else
            dir1.Parent |> isSubfolderOf dir2
        ...
        let files =
            ...
            |> Seq.fold
                (fun acc (d: DirectoryInfo) ->
                    ...
                    d |> copyRecursiveToWithFilter overwrite filter newDir |> (fun r -> r @ acc))
                []
    ...
    /// <param name="outputDir">The target directory to copy to</param>
    /// <param name="dir">The source directory to copy from</param>
    let copyRecursiveTo overwrite (outputDir: DirectoryInfo) (dir: DirectoryInfo) =
        copyRecursiveToWithFilter overwrite (fun _ _ -> true) outputDir dir
