...
module Shell =
        ...
        |> Seq.map (fun fileName ->
            ...
            if not originalExists then
                if not cachedFi.Exists then
                    ...
                    Trace.traceVerbose
                    <| sprintf "Original file %s does not exist, using cached file %s." fileName cached

                    ()
            else if not cachedFi.Exists || cachedFi.LastWriteTime <> fi.LastWriteTime then
                Trace.traceVerbose
                <| sprintf "Cached file %s doesn't exist or is not up to date. Copying file to cache." cached
    ...
    let appendTextFilesWithEncoding encoding newFileName files =
        ...
        if fi.Exists then
            failwithf "File %s already exists." fi.FullName
    ...
    let appendTextFiles newFileName files =
        appendTextFilesWithEncoding System.Text.Encoding.UTF8 newFileName files
    ...
    let compareFiles delete originalFileName compareFileName =
        ...
        let identical =
            if not (ori.Exists && comp.Exists && ori.Length = comp.Length) then
                false
            else
                ori.LastWriteTime = comp.LastWriteTime || FileInfo.contentIsEqualTo ori comp
        ...
        if not identical then
            ...
            if delete then
                ...
                Trace.traceVerbose <| sprintf "Files equal: %s" comp.FullName
    ...
    let generatePatchWithFindOldFileFunction lastReleaseDir patchDir srcFiles findOldFileF =
        ...
        for file in srcFiles do
            ...
            if not fi.Exists then
                Trace.traceVerbose <| sprintf "LastRelease has no file like %s" fi.FullName
    ...
    let copyRecursive2 method dir outputDir =
        ...
        let copyRecursiveWithFilter f =
            DirectoryInfo.copyRecursiveToWithFilter false f outputDirInfo dirInfo
    ...
    let replaceInFiles replacements files =
        Templates.replaceInFiles replacements files
    ...
    let regexReplaceInFileWithEncoding pattern (replacement: string) encoding file =
        ...
        let newContent =
            System.Text.RegularExpressions.Regex.Replace(oldContent, pattern, replacement)
    ...
    let mv src dest =
        ...
        | FileSystemInfo.File _, FileSystemInfo.Directory _ -> moveFile dest src
        | FileSystemInfo.Directory _, FileSystemInfo.Directory _ -> moveDir dest src
