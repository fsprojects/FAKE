...
module AssemblyInfoFile =
    ...
    let createCSharpWithConfig outputFileName attributes (config: AssemblyInfoFileConfig) =
        ...
        let attributeLines =
            [ yield! [ "// Auto-Generated by FAKE; do not edit"; "// <auto-generated/>" ]
              yield! (dependencies |> List.map (sprintf "using %s;"))
              yield! [ "" ]
              yield!
                  (attributes
                   |> Seq.toList
        ...
        let sourceLines =
            if generateClass then
                ...
                [ yield! (sprintf "namespace %s {" useNamespace) :: resharperSuppressions
                  yield! [ "    internal static class AssemblyVersionInformation {" ]
                  yield! consts
                  yield! [ "    }"; "}" ] ]
            else
                []
    ...
    let createVisualBasicWithConfig outputFileName attributes (config: AssemblyInfoFileConfig) =
        ...
        let attributeLines =
            [ yield! [ "'//' Auto-Generated by FAKE; do not edit"; "'//' <auto-generated/>" ]
              yield! (getDependencies attributes |> List.map (sprintf "Imports %s"))
              yield! [ "" ]
              yield!
                  (attributes
                   |> Seq.toList
