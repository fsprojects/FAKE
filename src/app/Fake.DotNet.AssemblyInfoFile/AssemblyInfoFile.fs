...
module AssemblyInfoFile =
    ...
    let createCSharpWithConfig outputFileName attributes (config : AssemblyInfoFileConfig) =
        ...
        let attributeLines =
            ...
            @ [ "" ]
            @ (attributes |> Seq.toList |> List.map (fun (attr : Attribute) -> sprintf "[assembly: %sAttribute(%s)]" attr.StaticPropertyName attr.StaticPropertyValue))
    ...
    let createFSharpWithConfig outputFileName attributes (config : AssemblyInfoFileConfig) =
        ...
        let sourceLines =
            [
                ...
                yield!
                    attributes
                    |> Seq.map (fun (attr : Attribute) -> sprintf "[<assembly: %sAttribute(%s)>]" attr.StaticPropertyName attr.StaticPropertyValue)
    ...
    let createVisualBasicWithConfig outputFileName attributes (config : AssemblyInfoFileConfig) =
        ...
        let attributeLines =
            ...
            @ [ "" ]
              @ (attributes
                 |> Seq.toList
                 |> List.map (fun (attr : Attribute) -> sprintf "<assembly: %sAttribute(%s)>" attr.StaticPropertyName attr.StaticPropertyValue))
