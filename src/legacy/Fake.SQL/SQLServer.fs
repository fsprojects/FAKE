...
[<System.Obsolete("FAKE0001 Use the Fake.Sql module instead.")>]
type ServerInfo = { Server: Server; ConnBuilder: SqlConnectionStringBuilder }
...
let getDatabaseNamesFromServer (serverInfo: ServerInfo) =
    getDatabasesFromServer serverInfo |> Seq.map (fun db -> db.Name)
...
let existDBOnServer serverInfo dbName =
    ...
    names |> Seq.iter (tracefn "  - %s ")

    names |> Seq.exists ((=) dbName)
...
let ReplaceDatabaseFiles connectionString targetDir files attachOptions =
    replaceDatabaseFiles connectionString attachOptions (fun _ ->
        files
...
let ReplaceDatabaseFilesWithCache connectionString targetDir cacheDir files attachOptions =
    replaceDatabaseFiles connectionString attachOptions (fun _ -> CopyCached targetDir cacheDir files)
...
let DropAndCreateDatabase connectionString =
    connectionString |> getServerInfo |> DropDb |> CreateDb |> Disconnect
