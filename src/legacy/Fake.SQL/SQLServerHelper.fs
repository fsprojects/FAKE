...
module SqlServerSmoHelper =
    ...
    let existDBOnServer serverInfo dbName =
        serverInfo |> getDatabaseNamesFromServer |> Seq.exists (fun d -> d = dbName)
    ...
    let DropDb serverInfo =
        if intitialCatalogExistsOnServer serverInfo then
            log
            <| sprintf "Dropping database %s on server %s" (getDBName serverInfo) (getServerName serverInfo)
    ...
    let CreateDb serverInfo =
        log
        <| sprintf "Creating database %s on server %s" (getDBName serverInfo) (getServerName serverInfo)
