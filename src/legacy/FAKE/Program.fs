...
try
    try
        ...
        let hasRemoveWarningEnvVar =
            System.Environment.GetEnvironmentVariable("FAKE_NO_LEGACY_WARNING") = "true"
        ...
        | Choice1Of2(fakeArgs) ->
            ...
            let isBoot, bootArgs =
                fakeArgs.Contains <@ Cli.Boot @>, fakeArgs.GetResults <@ Cli.Boot @>
            ...
            | false, false ->
                ...
                let envVars =
                    seq {
                        yield! fakeArgs.GetResults <@ Cli.EnvFlag @> |> Seq.map (fun name -> name, "true")
                        yield! fakeArgs.GetResults <@ Cli.EnvVar @>

                        if fakeArgs.Contains <@ Cli.Single_Target @> then
                            yield "single-target", "true"

                        if args.Target.IsSome then
                            yield "target", args.Target.Value
                    }

                //Get our fsiargs from somewhere!
                let fsiArgs =
                    match fakeArgs.GetResults <@ Cli.FsiArgs @>, args.Script, List.isEmpty buildScripts with
                    ...
                    | x :: xs, maybeScript, _ ->
                        ...
                        | Choice2Of2(msg) ->
                            ...
                            | None -> failwith (sprintf "Unable to parse --fsiargs.  %s." msg)
                ...
                if not (runBuildScriptWithFsiArgsAt printDetails fsiArgs envVars useCache) then
        ...
        | Choice2Of2(ex) ->
            ...
            if
                (cmdArgs.Length = 2 && paramIsHelp cmdArgs.[1])
            ...
            then
                ...
                | None ->
                    let buildScriptArg =
                        if cmdArgs.Length > 1 && cmdArgs.[1].EndsWith ".fsx" then
                            cmdArgs.[1]
                        else
                            Seq.head buildScripts
                    ...
                    let args =
                        CommandlineParams.parseArgs (
                            fakeArgs |> Seq.filter ((<>) buildScriptArg) |> Seq.filter ((<>) "details")
                        )
                    ...
                    if not (runBuildScript printDetails buildScriptArg fsiArgs args useCache) then
                        Environment.ExitCode <- 1
                    else if printDetails then
                        log "Ready."
                | Some handler -> handler.Interact()
    with exn ->
    ...
    if !TargetHelper.ExitCode.exitCode <> 0 then
        exit !TargetHelper.ExitCode.exitCode
