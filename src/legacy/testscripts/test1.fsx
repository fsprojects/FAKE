...
Target "blah" (fun _ ->
    ...
    let simpleWithParams =
        { simpleConfig with
            Parameters = Some [ ("Verbosity", "High"); ("dupe2", "wat") ] }
    ...
    let wcl =
        Some
            [ { ClassName = Some "WorkflowCentralLogger"
                AssemblyPath =
                  "C:\Program Files\Microsoft Team Foundation Server 12.0\Tools\Microsoft.TeamFoundation.Build.Server.Logger.dll"
                Parameters =
                  Some
                      [ "Verbosity", "Normal"
              ...
              Some
                  { ClassName = Some "WorkflowForwardingLogger"
                    AssemblyPath =
                      "C:\Program Files\Microsoft Team Foundation Server 12.0\Tools\Microsoft.TeamFoundation.Build.Server.Logger.dll"
                    Parameters = Some [ "Verbosity", "Normal" ] } ]
