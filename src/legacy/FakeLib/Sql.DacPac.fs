...
let validPaths =
    ...
    let getVsVersion path =
        (Path.GetDirectoryName path |> DirectoryInfo).Name |> int

    let sql =
        !!(ProgramFilesX86 </> @"Microsoft SQL Server\**\DAC\bin\SqlPackage.exe")
...
let defaultDeploymentArgs =
    { SqlPackageToolPath = validPaths |> List.tryHead |> defaultArg <| ""
      Action = Deploy
      Source = ""
      Destination = ""
      Timeout = 120
...
let deployDb setParams =
    ...
    if not (File.Exists args.SqlPackageToolPath) then
        let paths =
            if validPaths |> List.contains args.SqlPackageToolPath then
                validPaths
            else
    ...
    let result =
        ExecProcessWithLambdas
            (fun psi ->
                psi.Arguments <-
                    sprintf
                        """/Action:%s /SourceFile:"%s" /TargetConnectionString:"%s" %s /p:BlockOnPossibleDataLoss=%b /p:DropObjectsNotInSource=%b /p:CommandTimeout=%d /p:CreateNewDatabase=%b %s %s"""
                        action
                        args.Source
                        args.Destination
                        outputPath
                        args.BlockOnPossibleDataLoss
                        args.DropObjectsNotInSource
                        args.Timeout
                        args.RecreateDb
