...
module CMake =
    ...
    let FindExe exeName =
        ...
        |> (fun list ->
            if isUnix then
                ...
                List.append list [ Seq.singleton (ProgramFilesX86 @@ "CMake" @@ "bin") ])
        |> Seq.concat
    ...
    let private CallCMake toolPath binaryDir args timeout =
        ...
        if result <> 0 then
            failwithf "CMake failed with exit code %i." result
    ...
    let Generate setParams =
        ...
        // Not defaulted because it would prevent building multiple CMake projects in the same FAKE script.
        if isNullOrEmpty parameters.SourceDirectory then
            failwith "The CMake source directory is not set."

        let argsIfNotEmpty format values =
            List.filter (isNotNullOrEmpty) values |> List.map (sprintf format)
