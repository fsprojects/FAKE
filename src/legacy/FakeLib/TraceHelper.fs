...
[<System.Obsolete("use nuget 'Fake.Core.Trace' instead (open Fake.Core and use 'Trace.')")>]
let logf fmt =
    Printf.ksprintf (fun text -> postMessage (LogMessage(text, false))) fmt
...
let exceptionAndInnersToString (ex: Exception) =
    ...
    let rec printException (e: Exception) count =
        if (e :? TargetException && e.InnerException <> null) then
            ...
            if (count = 1) then
                bprintf sb "Exception Message:%s%s%s" e.Message nl delimeter
            else
                bprintf sb "%s%s%d)Exception Message:%s%s%s" nl nl count e.Message nl delimeter
...
let traceStartTaskUsing task description =
    ...
    { new IDisposable with
        member x.Dispose() = traceEndTask task description }
...
let Log message files =
    files |> Seq.iter (log << sprintf "%s%s" message)
