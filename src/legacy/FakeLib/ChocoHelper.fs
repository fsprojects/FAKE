...
module Choco =
    ...
    let private getPaths =
        ...
        if programDataPath |> isNotNullOrEmpty then
            [ Seq.singleton (programDataPath @@ "chocolatey" @@ "bin"); pathDirectories ]
        else
            [ pathDirectories ]
    ...
    let private callChoco exePath args timeout =
        ...
        let chocoExe =
            if not <| isNullOrEmpty exePath then
                exePath
            else
                let found = FindExe
        ...
        if result <> 0 then
            failwithf "choco failed with exit code %i." result
    ...
    let private getNuspecData parameters =
        ...
        let getFrameworkGroup (frameworkTags: (string * string) seq) =
            ...
            |> Seq.map (fun (frameworkVersion, tags) ->
                if isNullOrEmpty frameworkVersion then
    ...
    let private createChocolateyInstallPs1 (parameters: ChocoPackParams) outputDir =
        ...
        if
            isNullOrWhiteSpace parameters.Title
            || isNullOrWhiteSpace parameters.PackageDownloadUrl
        then
        ...
        let installContent =
            ...
            |> match parameters.InstallerType with
               | ChocolateyInstallerType.Zip ->
                   appendWithoutQuotes "Install-ChocolateyZipPackage $packageName $url $unzipLocation"
            ...
            |> appendIfTrueWithoutQuotes
                (isNotNullOrEmpty parameters.Checksum)
                ("-ChecksumType " + checksumTypeToString parameters.ChecksumType)
            |> appendIfTrueWithoutQuotes
                (isNotNullOrEmpty parameters.Checksum64)
    ...
    let private createChocolateyUninstallPs1 (parameters: ChocoPackParams) outputDir =
        ...
        if
            not (isNullOrWhiteSpace parameters.Title)
            && not (isNullOrWhiteSpace parameters.UninstallPath)
        then
            ...
            let uninstallContent =
                ...
                |> match parameters.InstallerType with
                   | ChocolateyInstallerType.Zip ->
                       appendWithoutQuotes "Uninstall-ChocolateyZipPackage $packageName $file"
                   | _ ->
                       appendWithoutQuotes "Uninstall-ChocolateyPackage $packageName $installerType $silentArgs $file"
                |> toText
    ...
    let Install (setParams: (ChocoInstallParams -> ChocoInstallParams)) (packages: string) =
        if packages |> isNullOrEmpty then
    ...
    let Pack setParams =
        ...
        |> FileHelper.MoveFile parameters.OutputDir
    ...
    let PackFromTemplate setParams nuspecPath =
        ...
        if nuspecPath |> isNullOrEmpty then
        ...
        if fileExists chocoUninstallPath then
    ...
    let Push setParams nupkgPath =
        if nupkgPath |> isNullOrEmpty then
