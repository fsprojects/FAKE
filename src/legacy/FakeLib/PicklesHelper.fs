...
let buildPicklesArgs parameters =
    let outputFormat =
        match parameters.OutputFileFormat with
        | DHTML -> "dhtml"
        | HTML -> "html"
        | Word -> "word"
        | JSON -> "json"
    ...
    let testResultFormat =
        match parameters.LinkedTestResultFiles with
        | [] -> None
        | _ ->
            match parameters.TestResultsFormat with
            | Nunit -> Some "nunit"
            | NUnit -> Some "nunit"
            | NUnit3 -> Some "nunit3"
            | XUnit -> Some "xunit"
    ...
    let result =
        ExecProcess
            (fun info ->
                ...
                info.Arguments <- parameters |> buildPicklesArgs)
            parameters.TimeOut
