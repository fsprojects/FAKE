...
let rec private buildPaths acc (input: SearchOption list) =
    ...
    | [] -> acc
    | Directory(name) :: t ->
        let subDirs = acc |> List.map (checkSubDirs false name) |> List.concat
        buildPaths subDirs t
    | Drive(name) :: t ->
        let subDirs = acc |> List.map (checkSubDirs true name) |> List.concat
        buildPaths subDirs t
    | Recursive :: [] ->
        let dirs =
        ...
        Seq.collect
            (fun dir ->
                if Directory.Exists(Path.Combine(dir, pattern)) then
                    seq { yield Path.Combine(dir, pattern) }
                else
                    try
                        Directory.EnumerateFiles(dir, pattern)
                    with :? System.IO.PathTooLongException as ex ->
                        Array.toSeq [||])
            acc
        |> Seq.toList
