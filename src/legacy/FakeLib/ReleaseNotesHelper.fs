...
let parseVersions =
    ...
    fun line ->
        ...
        if not nugetVersion.Success then
            failwithf "Unable to parse valid NuGet version from release notes (%s)." line
...
let private parseAllComplexReleaseNotes (text: seq<string>) =
        ...
        | Some(header, (notes, rest)) ->
            ...
            let newReleaseNotes =
                ReleaseNotes.New(
                    ...
                    nugetVer.Value,
                    date,
                    notes |> List.filter isNotNullOrEmpty |> List.rev
                )
    ...
    loop
        []
        (text
         |> Seq.map (trimStartChars [| ' '; '*' |] >> trimEndChars [| ' ' |])
         |> Seq.toList)
...
let parseAllReleaseNotes (data: seq<string>) =
    ...
    | h :: _ ->
        ...
        | Simple -> data |> Seq.map parseSimpleReleaseNotes |> Seq.toList
...
let parseReleaseNotes (data: seq<string>) =
    data |> parseAllReleaseNotes |> Seq.head
...
let LoadReleaseNotes fileName =
    System.IO.File.ReadLines fileName |> parseReleaseNotes
