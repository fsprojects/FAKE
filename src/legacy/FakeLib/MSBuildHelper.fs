...
let msBuildExe =
    ...
    let exactPathOrBinaryOnPath tool input =
        if FileSystemHelper.isDirectory input && Directory.Exists input then
            input </> tool
        else
            input
    ...
    let foundExe =
        ...
        | false, _ ->
            ...
            let configIgnoreMSBuild =
                if
                    "true"
                        .Equals(ConfigurationManager.AppSettings.["IgnoreMSBuild"], StringComparison.OrdinalIgnoreCase)
                then
                    Some ""
                else
                    None
            ...
            let findOnVSPathsThenSystemPath =
                ...
                let vsVersionPaths =
                    defaultArg
                        (EnvironmentHelper.environVarOrNone "VisualStudioVersion"
...
let internal unescapeMSBuildSpecialChars s =
    ...
    replExpr.Replace(
        s,
        new Text.RegularExpressions.MatchEvaluator(fun _match ->
...
let internal getReferenceElements elementName projectFileName (doc: XDocument) =
    ...
    |> Seq.map (fun e ->
        ...
        let fileName =
            if
                value.StartsWith(".." + directorySeparator)
                || (not <| value.Contains directorySeparator)
            then
...
let rec getProjectReferences (projectFileName: string) =
    if projectFileName.EndsWith ".sln" then
        Set.empty
...
let getAllParameters
    ...
    fileLoggers
    binaryLoggers
    distributedFileLoggers
    properties
    =
    if isUnix then
        [ targets; tools; verbosity; noconsolelogger; warnAsError; nowarn ]
        @ fileLoggers
        @ binaryLoggers
        @ distributedFileLoggers
        @ properties
...
let serializeMSBuildParams (p: MSBuildParams) =
    ...
    let properties =
        ("RestorePackages", p.RestorePackagesFlag.ToString()) :: p.Properties
        |> List.map (fun (k, v) -> Some("p", sprintf "%s=\"%s\"" k v))
        ...
        let serializeLogger fl =
            ...
            sprintf
                ...
                (match fl.Parameters with
                 | None -> ""
                 | Some ps -> ps |> List.map (fun p -> sprintf "%s;" (logParams p)) |> String.concat "")
...
let mutable MSBuildLoggers =
    [ ErrorLoggerName ] |> List.map (fun a -> sprintf "%s,\"%s\"" a pathToLogger)
...
let build setParams project =
    ...
    let msBuildParams = MSBuildDefaults |> setParams

    let argsString = msBuildParams |> serializeMSBuildParams

    let errorLoggerParam =
        MSBuildLoggers |> List.map (fun a -> Some("logger", a)) |> serializeArgs
...
let MSBuildWithProjectProperties
    outputPath
    (targets: string)
    ...
    let output =
        if isNullOrEmpty outputPath then
            ...
            outputPath |> FullName |> trimSeparator

    let properties =
        if isNullOrEmpty output then
            properties
        else
            fun x -> ("OutputPath", output) :: (properties x)
    ...
    if isNotNullOrEmpty output then
        !!(outputPath @@ "/**/*.*") |> Seq.toList
    else
        []
...
let MSBuild outputPath targets properties projects =
    MSBuildWithProjectProperties outputPath targets (fun _ -> properties) projects
...
let MSBuildWithDefaults targets projects =
    MSBuild null targets [ "Configuration", "Release" ] projects
...
let BuildWebsiteConfig outputPath configuration projectFile =
    ...
    MSBuild null "Build" [ "Configuration", configuration ] [ projectFile ]
    |> ignore
...
let BuildWebsite outputPath projectFile =
    BuildWebsiteConfig outputPath "Debug" projectFile
...
[<System.Obsolete("Use Fake.DotNet.MSBuild instead")>]
let BuildWebsitesConfig outputPath configuration projectFiles =
    Seq.iter (BuildWebsiteConfig outputPath configuration) projectFiles
