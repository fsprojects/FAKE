...
let private yarnFileName =
    ...
    | _ ->
        ...
        | 0 when not proc.StandardOutput.EndOfStream -> proc.StandardOutput.ReadLine()
        | _ -> "/usr/bin/yarn"
...
let run yarnParams =
    ...
    if not ok then
        failwith (sprintf "'yarn %s' task failed" arguments)
...
[<System.Obsolete("FAKE0001 Use the Fake.JavaScript.Yarn module instead")>]
let Yarn setParams = defaultYarnParams |> setParams |> run
