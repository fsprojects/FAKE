...
let MageSerializeParams (action: MageCall) (mp: MageParams) =
    ...
    let name =
        if isNullOrEmpty mp.Name then
            ""
        else
            "-n \"" + mp.Name + "\""

    let iconFile =
        if isNullOrEmpty mp.IconFile then
            ""
        else
            "-if \"" + mp.IconFile + "\""
    ...
    let fromDir =
        if isNullOrEmpty mp.FromDirectory then
            ""
        else
            "-fd " + mp.FromDirectory

    let manifest =
        if isNullOrEmpty mp.Manifest then
            ""
        else
            "-appm " + mp.Manifest
    ...
    let password =
        ...
        | None -> ""
        | Some(p) ->
            if isNullOrEmpty certFile then ""
    ...
    let trustlevel =
        if isNullOrEmpty trustlevelStr then
            ""
        else
            "-tr " + trustlevelStr
    ...
    let providerUrl =
        if isNullOrEmpty mp.ProviderURL then
            ""
        else
            "-pu \"" + mp.ProviderURL + "\""
...
let mageCall (action: MageCall) (mp: MageParams) =
    ...
    if result <> 0 then
        failwithf "Error during mage call "
...
let MageCreateApp (mp: MageParams) = mageCall NewApp mp
...
let MageUpdateApp (mp: MageParams) = mageCall UpdateApp mp
...
let MageSignManifest (mp: MageParams) = mageCall Sign mp
...
let MageDeployApp (mp: MageParams) = mageCall Deploy mp

[<System.Obsolete("Open Fake.DotNet instead (FAKE0001 - package: Fake.DotNet.Mage, function: updateDeploy")>]
let MageUpdateDeploy (mp: MageParams) = mageCall UpdateDeploy mp
...
let MageSignDeploy (mp: MageParams) = mageCall SignDeploy mp
