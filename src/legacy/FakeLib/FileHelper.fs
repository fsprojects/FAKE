...
let setDirectoryReadOnly readOnly (dir: DirectoryInfo) =
    if dir.Exists then
        let isReadOnly =
            dir.Attributes &&& FileAttributes.ReadOnly = FileAttributes.ReadOnly
...
let SetReadOnly readOnly (files: string seq) =
    ...
    |> Seq.iter (fun file ->
        ...
        if fi.Exists then
            ...
            file |> directoryInfo |> setDirectoryReadOnly readOnly)
...
let DeleteDir path =
    ...
    if dir.Exists then
        ...
        !! "/**/*.*" |> SetBaseDir dir.FullName |> (SetReadOnly false)
...
let CreateDir path =
    ...
    if not dir.Exists then
        ...
        logfn "%s already exists." dir.FullName
...
let CreateFile fileName =
    ...
    if not file.Exists then
        ...
        logfn "%s already exists." file.FullName
...
let CopyWithSubfoldersTo target files =
    let copyFiles dir inc =
        Seq.iter (CopyFileWithSubfolder dir target) inc
...
let CopyRecursive dir outputDir =
    copyRecursive (directoryInfo dir) (directoryInfo outputDir)
...
let changeExt extension fileName =
...
let hasExt extension fileName =
    System.String.Equals(ext fileName, extension, System.StringComparison.InvariantCultureIgnoreCase)
