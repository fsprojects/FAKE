...
type ProjectFile(projectFileName: string, documentContent: string) =
    ...
    [<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
    static member FromFile(projectFileName) =
        new ProjectFile(projectFileName, ReadFileAsString projectFileName)

    /// Saves the project file
    [<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
    member x.Save(?fileName) =
        document.Save(defaultArg fileName projectFileName)
...
let findMissingFiles templateProject projects =
    ...
    |> Seq.map (fun ps ->
        ...
        let unorderedFiles =
            if not <| isFSharpProject templateProject then
                []
            else if not <| Seq.isEmpty missingFiles then
                []
            else
                let remainingFiles =
                    ps.Files |> List.filter (fun file -> Set.contains file templateFilesSet)

                if remainingFiles.Length <> templateFiles.Length then
                    []
                else
...
let removeCompileNodesWithMissingFiles includeExistsF (project: ProjectFile) =
    ...
    let missingFiles =
        seq {
            for filePath in project.Files do
                ...
                let includePath =
                    Globbing.normalizePath (IO.Path.Combine([| projectDir; filePath |]))

                if not (includeExistsF (includePath)) then
                    yield filePath
        }
...
let removeContentNodesWithMissingFiles includeExistsF (project: ProjectFile) =
    ...
    let missingFiles =
        seq {
            for filePath in project.ContentFiles do
                // We have to normalize the path, because csproj can have win style directory separator char on Mono too
                // Xbuild handles them, so we do too http://www.mono-project.com/archived/porting_msbuild_projects_to_xbuild/#paths
                let includePath =
                    Globbing.normalizePath (IO.Path.Combine([| projectDir; filePath |]))
...
let RemoveCompileNodesWithMissingFiles project =
    let newProject =
        removeCompileNodesWithMissingFiles System.IO.File.Exists (ProjectFile.FromFile project)
...
let RemoveContentNodesWithMissingFiles project =
    let newProject =
        removeContentNodesWithMissingFiles System.IO.File.Exists (ProjectFile.FromFile project)
