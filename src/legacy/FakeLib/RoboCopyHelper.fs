...
let private roboCopyWithOptions (source: string) (destination: string) (options: RoboCopyOptions) =
    let args =
        ...
        + if options.Mirror then " /MIR /IT" else " /IT"
    ...
    | (exitcode, message) when exitcode < 2 ->
        tracefn "Succeeded in RoboCopy From: %s To: %s \n%s\n" source destination message
        |> ignore
    | (exitcode, message) when exitcode < 8 ->
        traceImportant
        <| sprintf "Important notice in RoboCopy From: %s To: %s \n%s\n" source destination message
        |> ignore
