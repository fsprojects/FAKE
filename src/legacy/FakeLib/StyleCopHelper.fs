...
let StyleCop (setParams: StyleCopParams -> StyleCopParams) =
    ...
    let analyser =
        StyleCopConsole(param.SettingsFile, true, param.OutputFile, null, true)
    ...
    let visualStudioFiles sourcePaths =
        VisualStudioFiles(Seq.ofList sourcePaths, param.RecursiveSearch)
    ...
    let addSourceFile (codeProject: CodeProject) (sourceFile: CSharpSourceFile) =
        ...
        analyser.Core.Environment.AddSourceCode(codeProject, sourceFile.FilePath, null)
        |> ignore
    ...
    let rec addSourceFiles id (files: List<CSharpSourceFile>) =
        combineCodeProjectFromFiles id files (fun id head ->
    ...
    let rec addSolutionFiles id (files: List<SolutionFile>) =
        ...
        |> List.collect (fun f ->
            f.Load()
            addProjectFiles id (List.ofSeq f.CSharpProjectFiles))
    ...
    let codeProjects =
        ...
        |> (fun projects ->
            ...
            let codeProjectsFromProjects =
                addProjectFiles (List.length projects) (List.ofSeq projectFiles.CSharpProjectFiles)
        ...
        |> (fun projects ->
            ...
            let codeProjectsFromSolutions =
                addSolutionFiles (List.length projects) (List.ofSeq solutionFiles.SolutionFiles)
