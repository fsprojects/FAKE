...
module Replacements =
    ...
    let win8Replacements =
        [ "4.0:{F6D90F11-9C73-11D3-B32E-00C04F990BB4}:'Microsoft XML, v4.0'.DOMDocument",
          "6.0:{88D96A05-F192-11D4-A65F-0040963251E5}:'Microsoft XML, v6.0'.DOMDocument60"
          "4.0:{2933BF80-7B36-11D2-B20E-00C04F983E60}:'Microsoft XML, v4.0'",
          "6.0:{2933BF80-7B36-11D2-B20E-00C04F983E60}:'Microsoft XML, v6.0'"
          "{F6D90F11-9C73-11D3-B32E-00C04F990BB4}:'Microsoft XML, v6.0'.DOMDocument",
          "{88D96A05-F192-11D4-A65F-0040963251E5}:'Microsoft XML, v6.0'.DOMDocument60" ]

    [<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
    let Win8ToWin7 (s: string) =
        if isWin8 then
            win8Replacements |> Seq.fold (fun (s: string) (r, p) -> s.Replace(p, r)) s
        else
            s
    ...
    let Win7ToWin8 (s: string) =
        if isWin8 then
            win8Replacements |> Seq.fold (fun (s: string) (p, r) -> s.Replace(p, r)) s
        else
            s
    ...
    let replaceShortcuts (s: string) =
        shortcutReplacements |> Seq.fold (fun (s: string) (r, p) -> s.Replace(p, r)) s
...
let getNAVPath navClientVersion =
    (directoryInfo (getNAVClassicPath navClientVersion)).Parent.FullName
...
let getNAVServicePath navClientVersion =
    let navServiceRootPath =
        ...
        match navClientVersion with
        | "601"
        | "602" -> getRegistryValue HKEYLocalMachine subKey "Path"
        | "700"
        | "701"
        | "800"
        | "900"
        ...
        | "1100"
        | "1300"
        | "1400" -> getRegistryValue64 HKEYLocalMachine subKey "Path"
        | _ -> failwithf "Unknown NAV-Version (Service) %s" navClientVersion
...
let private import connectionInfo fileName =
    ...
    let args =
        sprintf
            "command=importobjects, file=\"%s\", logfile=\"%s\", servername=\"%s\", database=\"%s\""
            importFile.FullName
            (FullName connectionInfo.TempLogFile)
...
let private export connectionInfo filter fileName =
    ...
    let args =
        sprintf
            "command=exportobjects, file=\"%s\", logfile=\"%s\", filter=\"%s\", servername=\"%s\", database=\"%s\""
            fi.FullName
            (FullName connectionInfo.TempLogFile)
            filter
            connectionInfo.ServerName
...
let CompileWithFilter filter (connectionInfo: DynamicsNavParams) =
    ...
    let args =
        sprintf
            "command=compileobjects, filter=\"Compiled=0;%s\", logfile=\"%s\", servername=\"%s\", database=\"%s\""
            filter
            (FullName connectionInfo.TempLogFile)
            connectionInfo.ServerName
            connectionInfo.Database
    ...
    if
        ...
        <> ExecProcess
            ...
            connectionInfo.TimeOut
    then
        reportError (sprintf "Compile with filter %s failed." filter) connectionInfo.TempLogFile
...
let CompileAll connectionInfo =
    ...
    let args =
        sprintf
            "command=compileobjects, filter=\"Compiled=0\", logfile=\"%s\", servername=\"%s\", database=\"%s\""
            (FullName connectionInfo.TempLogFile)
            connectionInfo.ServerName
            connectionInfo.Database
...
let FullCompile connectionInfo =
    ...
    let args =
        sprintf
            "command=compileobjects, logfile=\"%s\", servername=\"%s\", database=\"%s\""
            (FullName connectionInfo.TempLogFile)
            connectionInfo.ServerName
            connectionInfo.Database
...
let RunCodeunit connectionInfo (codeunitID: int) =
    ...
    let args =
        sprintf
            "-consolemode \"DynamicsNAV://%s:%d/%s/%s/runcodeunit?codeunit=%d\" -ShowNavigationPage:0"
            connectionInfo.ServerName
            connectionInfo.Port
    ...
    if exitCode <> 0 && exitCode <> 255 then
        reportError
            (sprintf "Running codeunit %d failed with ExitCode %d" codeunitID exitCode)
...
let RunCodeunitWithSettings connectionInfo settingsFile (codeunitID: int) =
    ...
    let args =
        sprintf
            "-settings:\"%s\" -consolemode \"DynamicsNAV://%s:%d/%s/%s/runcodeunit?codeunit=%d\" -ShowNavigationPage:0"
            settingsFile
            connectionInfo.ServerName
            connectionInfo.Port
            connectionInfo.ServiceTierName
...
let OpenPage connectionInfo pageNo =
    ...
    let protocol =
        sprintf
            @"dynamicsnav://%s:%d/%s/%s/runpage?page=%d"
            connectionInfo.ServerName
            connectionInfo.Port
            connectionInfo.ServiceTierName
            connectionInfo.Company
    ...
    let closedProcesses =
        ...
        |> List.map (fun p ->
            try
                p.Kill()
            with _ ->
                traceImportant <| sprintf "Could not kill all NAV process %O" p.Id)

    if closedProcesses = [] && raiseExceptionIfNotFound then
        failwith "Could not kill all NAV processes"
...
let analyzeTestResults fileName =
    ...
    if tryFindNext "TestSuiteNotFound;" messages <> None then
        None
    else
        ...
        | Some suiteName ->
            let rec getTests (messages: string seq) =
                ...
                let currentMessages =
                    messages |> Seq.takeWhile (fun x -> x.StartsWith "EndOfTest;" |> not)
                ...
                if Seq.isEmpty messages then
                    ...
                    | Some testName ->
                        let status =
                            match
                                currentMessages
                                |> Seq.tryFind (fun x -> x.StartsWith "Error;" || x.StartsWith "Ignored;")
                            with
                        ...
                        { Name = testName; RunTime = runTime; Status = status } :: getTests messages
                ...
                | _ ->
                    ...
                    | "Failure" ->
                        Failure(
                            ...
                            + testCodeunit
                            + ", "
                            + node.SelectSingleNode("Function").InnerText
                            + ")\n"
                        )
                    | _ -> Ok
...
let StartNavServiceTier serverMode navClientVersion =
    ...
    | NavisionServerType.SqlServer ->
        match navClientVersion with
        | "700" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV70"
        | "701" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV71"
        | "800" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV80"
        | "900" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV90"
        | "1000" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV100"
        | "1100" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV110"
        | "1300" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV130"
        | "1400" -> StartService "MicrosoftDynamicsNavServer$DynamicsNAV140"
