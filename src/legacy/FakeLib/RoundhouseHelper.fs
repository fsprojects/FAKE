...
let private getIntParam k (v: int) = Some(k, v.ToString())
...
let private formatArgs args pre split delim =
    ...
    |> Seq.map (function
        | None -> ""
        | Some(k, v) -> pre + k + (if isNullOrEmpty v then "" else sprintf "%s%s" split v))
    |> separated delim

let private serializeArgs args = formatArgs args "/" "=" " "
...
let private getParamPairs (rh: RoundhouseParams) =
    ...
    let runAfterOtherTimeFolderPath =
        getStringParam "ra" rh.RunAfterOtherAnyTimeScriptsFolderName

    let runAfterCreateFolderPath =
        getStringParam "racd" rh.RunAfterCreateDatabaseFolderName
    ...
    [ dbName
      ...
      restoreFilePath
      alterFolderPath
      runAfterOtherTimeFolderPath
      runAfterCreateFolderPath
      runBeforeUpFolderPath
      upFolderPath
      runFirstAfterUpdateFolderPath
      funcFolderPath
      viewsFolderPath
      sprocsFolderPath
      indexFolderPath
      permissionsFolderPath
      drop
      simple
      transaction
      restore
