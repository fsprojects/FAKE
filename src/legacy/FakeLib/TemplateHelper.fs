...
let loadTemplates seq =
    Seq.map (fun fileName -> fileName, ReadFile fileName) seq
...
let saveFiles =
    Seq.iter (fun (fileName, file) -> WriteFile fileName (Seq.toList file))
...
let processTemplates replacements files =
    files |> loadTemplates |> replaceKeywords replacements |> saveFiles
