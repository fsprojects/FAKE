...
type WatchChangesOption = { IncludeSubdirectories: bool }
...
let private calcDirsToWatch fileIncludes =
    let dirsToWatch =
        ...
        |> Seq.map (fun file -> Globbing.getRoot fileIncludes.BaseDirectory file)

    // remove subdirectories from watch list so that we don't get duplicate file watchers running
    dirsToWatch
    |> Seq.filter (fun d -> dirsToWatch |> Seq.exists (fun p -> d.StartsWith p && p <> d) |> not)
    |> Seq.toList
        ...
        lock unNotifiedChanges (fun () ->
            if not (Seq.isEmpty !unNotifiedChanges) then
                let changes =
                    ...
                    |> Seq.map (fun (name, changes) -> changes |> Seq.sortBy (fun c -> c.Status) |> Seq.head)
