...
let private XDeclaration version encoding standalone =
    XDeclaration(version, encoding, standalone)

let private XName expandedName = XName.Get(expandedName)

let private XDocument xdecl content =
    XDocument(xdecl, content |> Seq.map (fun v -> v :> obj) |> Seq.toArray)

let private XElement expandedName content =
    XElement(XName expandedName, content |> Seq.map (fun v -> v :> obj) |> Seq.toArray) :> obj

let private XAttribute expandedName value =
    XAttribute(XName expandedName, value) :> obj

let private XAttributeXName expandedName value =
    Linq.XAttribute(expandedName, value) :> obj

/// Sparkel namespace used for RSS extension
let private sparkle =
    XNamespace.Get("http://www.andymatuschak.org/xml-namespaces/sparkle")
...
let writeAppcast (path: string) (cast: Appcast) =
    let toXml (cast: Appcast) =
        ...
        let item (e: AppcastItem) =
            let appendMinimumVersion =
                appendSome e.minimumSystemVersion (fun d -> XAttributeXName (sparkle + "minimumSystemVersion") d)

            let appendSig =
                appendSome e.dsaSignature (fun d -> XAttributeXName (sparkle + "dsaSignature") d)
