...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
let regAsmToolPath = !!(TargetPlatformPrefix + "/**/RegAsm.exe") |> getNewestTool
...
let RegAsm setParams lib =
    ...
    let args =
        if parameters.ExportTypeLibrary then
...
let public RegisterAssembliesWithCodebase workingDir (assemblies: string seq) =
    use __ =
        traceStartTaskUsing "Regasm with codebase" "Registering assemblies with codebase, expect warnings"
            ...
            let! regAsmResult =
                asyncShellExec
                    { defaultParams with
                        ...
                        CommandLine =
                            (sprintf "\"%s\" /tlb:%s /codebase" assembly ((Path.GetFileName assembly) + ".tlb")) }

            if regAsmResult <> 0 then
                failwith (sprintf "Register %s with codebase failed" assembly)
...
let public UnregisterAssemblies workingDir (assemblies: string seq) =
    use __ =
        traceStartTaskUsing "Regasm /unregister with codebase" "Registering assemblies with codebase, expect warnings"
            ...
            let! regAsmResult =
                asyncShellExec
                    { defaultParams with
                        ...
                        CommandLine =
                            (sprintf
                                "\"%s\" /tlb:%s /codebase /unregister"
                                assembly
                                ((Path.GetFileName assembly) + ".tlb")) }

            if regAsmResult <> 0 then
                failwith (sprintf "Unregister %s with codebase failed" assembly)
