...
let validateParams param =
    if param.AuthToken = "" then
        failwith "You must provide your auth token"

    if param.RoomId = "" then
        failwith "You must specify which room to notify"

    if param.From = "" then
        failwith "You must specify the name to send the message from"

    if param.Message = "" then
        failwith "You must provide a message to send"
...
let HipChatNotification (setParams: HipChatNotificationParams -> HipChatNotificationParams) =
    let sendNotification param =
        ...
        |> fun curlData ->
            String.Format(
                "-s -d {0} https://api.hipchat.com/v1/rooms/message?auth_token={1}&format=json",
                curlData,
                param.AuthToken
            )
        |> fun curlArgs -> Shell.Exec("curl", curlArgs)

    HipChatNotificationDefaults |> setParams |> validateParams |> sendNotification
