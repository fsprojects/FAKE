...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
let getIncludedAssembly scriptContents =
    extractDirectives rAssemblyRegex scriptContents

[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
let getSearchPaths scriptContents =
    extractDirectives searchPathRegex scriptContents
            ...
            if path.StartsWith ".fake" then
                ...
                let path =
                    if Path.IsPathRooted path then
                        ...
                        | None ->
                            failwithf
                                "Could not find script '%s' in any paths searched. Searched paths:\n%A"
                                path
                                searchPaths
                        | Some x -> x
...
let getScriptHash pathsAndContents fsiOptions =
    ...
    let paths =
        pathsAndContents
        |> Seq.map (fun x -> x.Location |> EnvironmentHelper.normalizePath)
        |> String.concat "\n"
...
module internal Cache =
    ...
    type AssemblyInfo = { Location: string; FullName: string; Version: string }

    type CacheConfig = { Assemblies: AssemblyInfo seq }
...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
type FsiArgs =
    | FsiArgs of string list * string * string list
...
let executeFSI workingDirectory script env =
    let (result, messages) =
        ExecProcessRedirected (fsiStartInfo script workingDirectory env) TimeSpan.MaxValue
...
let executeFSIWithArgs workingDirectory script extraFsiArgs env =
    let result =
        ExecProcess (FsiStartInfo workingDirectory (FsiArgs(extraFsiArgs, script, [])) env) TimeSpan.MaxValue
...
let executeFSIWithArgsAndReturnMessages workingDirectory script extraFsiArgs env =
    let (result, messages) =
        ExecProcessRedirected
            (fun startInfo -> FsiStartInfo workingDirectory (FsiArgs(extraFsiArgs, script, [])) env startInfo)
            TimeSpan.MaxValue
...
let executeFSIWithScriptArgsAndReturnMessages script (scriptArgs: string[]) =
    let (result, messages) =
        ExecProcessRedirected
            (fun si -> FsiStartInfo "" (FsiArgs([], script, scriptArgs |> List.ofArray)) [] si)
            TimeSpan.MaxValue
...
let hashRegex =
    Text.RegularExpressions.Regex(
        "(?<script>.+)_(?<hash>[a-zA-Z0-9]+)(\.dll|_config\.xml|_warnings\.txt)$",
        ...
        if cacheFilesExistAndAreValid then
            let currentlyLoaded =
                ...
                |> Seq.map (fun (name, group) ->
                    name,
                    group
                    |> Seq.sortByDescending (fun (_, a) -> a.GetName().Version)
                    |> Seq.head
                    |> snd)
                |> dict
            ...
            let loadedAssemblies =
                ...
                |> Seq.choose (fun assemInfo ->
                    try
                        let name = AssemblyName(assemInfo.FullName)
                    ...
                    with ex ->
                        if printDetails then
                            tracef "Unable to find assembly %A" assemInfo
            ...
            let assemVersionValidCount =
                loadedAssemblies
                |> Seq.filter (fun (assemInfo, assem) -> assem.GetName().Version.ToString() = assemInfo.Version)
                |> Seq.length
            ...
            AppDomain.CurrentDomain.add_AssemblyResolve (
                new ResolveEventHandler(fun _ ev ->
                    ...
                    | true, a ->
                        if printDetails then
                            tracefn "Redirect assembly load to known assembly: %s" ev.Name
                    ...
                    | _ ->
                        ...
                        match
                            ...
                            |> Seq.tryFind (fun asem ->
                                ...
                                n.Name = name.Name
                                && (isNull token
                                    || // When null accept what we have.
                                    n.GetPublicKeyToken() = token))
                        with
                        | Some(asem) ->
                        ...
                        | _ ->
                            if
                                not (ev.Name.StartsWith("FSharp.Compiler.Service.resources"))
                                && not (ev.Name.StartsWith("FSharp.Compiler.Service.MSBuild"))
                            then
                                if printDetails then
                                    traceFAKE "Could not resolve '%s'" ev.Name
...
let nameParser scriptFileName =
    ...
    let fullName i =
        sprintf "%s%s>.$FSI_%s%s" startString i i endString

    let exampleName = fullName "0001"

    let parseName (n: string) =
        if
            n.Length >= exampleName.Length
            && n.Substring(0, startString.Length) = startString
            && n.Substring(n.Length - endString.Length) = endString
        then
...
let private runScriptCached printDetails cacheInfo out err =
    ...
    try
        use execContext =
            Fake.Core.Context.FakeExecutionContext.Create true cacheInfo.ScriptFilePath []
...
let private handleCaching printDetails (session: IFsiSession) fsiErrorOutput (cacheDir: DirectoryInfo) cacheInfo =
    try
        let wishName =
            "FAKE_CACHE_"
            + Path.GetFileNameWithoutExtension cacheInfo.ScriptFileName
            + "_"
            + cacheInfo.ScriptHash
    ...
    with ex ->
        ...
        if File.Exists cacheInfo.AssemblyWarningsPath then
            ...
            try
                File.Delete cacheInfo.AssemblyWarningsPath
            with _ ->
                ()
...
let private runScriptUncached (useCache, fsiOptions) printDetails (cacheInfo: CacheInfo) out err =
    ...
    let cacheDir =
        DirectoryInfo(Path.Combine(Path.GetDirectoryName(cacheInfo.ScriptFilePath), ".fake"))
            ...
            if not <| List.isEmpty oldFiles then
                ...
                |> function
                    | true ->
                        traceError (
                            sprintf
                                "Unable to invalidate cache for '%s', please delete the .fake folder!"
                                cacheInfo.ScriptFileName
                        )
                    | _ -> ()
            else if printDetails then
    ...
    if printDetails && doTrace then
        ...
        try
            ...
            if logToFile then
                if System.IO.File.Exists svclogFile then
                    System.IO.File.Delete svclogFile
    ...
    try
        try
            use execContext =
                Fake.Core.Context.FakeExecutionContext.Create false cacheInfo.ScriptFilePath []
        ...
        if useCache && not cacheInfo.IsValid then
            ...
            if doCaching then
                ...
                with e ->
                    // See https://github.com/fsharp/FAKE/pull/1534
                    traceFAKE
                        "Error in FAKE-Caching (might be a bug in the runtime, use the no-cache option to get rid of this warning): %O"
                        e
...
let internal runFAKEScriptWithFsiArgsAndRedirectMessages
    printDetails
    (FsiArgs(fsiOptions, scriptPath, scriptArgs))
    env
    onErrMsg
    onOutMsg
    useCache
    =
    if printDetails then
        traceFAKE "Running Buildscript: %s" scriptPath

    if printDetails then
        System.AppDomain.CurrentDomain.add_AssemblyResolve (
            new System.ResolveEventHandler(fun _ e ->
                if
                    not (e.Name.StartsWith("FSharp.Compiler.Service.resources"))
                    && not (e.Name.StartsWith("FSharp.Compiler.Service.MSBuild"))
                then
...
let internal onMessage isError =
    let printer =
        if isError && TraceListener.importantMessagesToStdErr then
            eprintf
        else
            printf
...
let executeBuildScriptWithArgsAndFsiArgsAndReturnMessages script (scriptArgs: string[]) (fsiArgs: string[]) useCache =
    ...
    let result =
        runFAKEScriptWithFsiArgsAndRedirectMessages
            true
            (FsiArgs(fsiArgs |> List.ofArray, script, scriptArgs |> List.ofArray))
            []
            (appendMessage true)
            (appendMessage false)
            useCache
...
let runBuildScriptWithFsiArgsAt printDetails (FsiArgs(fsiOptions, script, scriptArgs)) env useCache =
    runFAKEScriptWithFsiArgsAndRedirectMessages
        printDetails
        (FsiArgs(fsiOptions, script, scriptArgs))
        env
        (onMessage true)
        (onMessage false)
        useCache
