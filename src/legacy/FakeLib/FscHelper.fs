...
let fscList (srcFiles: string list) (opts: string list) : int =
    ...
    let optsArr =
        ...
        if Seq.exists (fun e -> e = "-o" || e.StartsWith("--out:")) opts then
            opts @ srcFiles
        // But if it's not, then figure out what it should be.
        else
            let outExt =
                if Seq.exists (fun e -> e = "-a" || e = "--target:library") opts then
                    ".dll"
                else
                    ".exe"
...
let fsc (setParams: FscParams -> FscParams) (inputFiles: string list) : int =
    ...
    let references =
        let refs = fscParams.References |> List.map (fun r -> sprintf "--reference:%s" r)

        let isNonDefaultFramework =
            fscParams.References
            |> List.exists (fun r ->
                r.IndexOf("FSharp.Core.dll", System.StringComparison.InvariantCultureIgnoreCase)
                >= 0
                || r.IndexOf("mscorlib.dll", System.StringComparison.InvariantCultureIgnoreCase)
                   >= 0)

        if isNonDefaultFramework then
            "--noframework" :: refs
        else
            refs
