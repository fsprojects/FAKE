...
let private validateParams param : HockeyAppUploadParams =
    if param.ApiToken = "" then
        failwith "You must provide your API token"

    if param.File = "" then
        failwith "You must provide an app file to upload"
    ...
    if not (String.IsNullOrEmpty param.Dsym) then
        if
            not (
                param.Dsym.EndsWith(".dsym.zip", StringComparison.InvariantCultureIgnoreCase)
                || param.Dsym.Equals("mapping.txt", StringComparison.InvariantCultureIgnoreCase)
            )
        then
            failwith "DSYM files should only be: IOS: *.dsym.zip  Android: mapping.txt"

        if
            param.File.EndsWith(".ipa")
            && not (param.Dsym.EndsWith(".dsym.zip", StringComparison.InvariantCultureIgnoreCase))
        then
            failwith "DSYM for an .ipa file can only only be: *.dsym.zip"

        if
            param.File.EndsWith(".apk")
            && not (param.Dsym.Equals("mapping.txt", StringComparison.InvariantCultureIgnoreCase))
        then
...
let private validateVersionParams (param: HockeyAppVersionParams) =
    ...
    if param.Version = "" then
        failwith "You must provide a version"

    if param.AppId = "" then
        failwith "You must provide an app id"

    param

/// [omit]
let private toCurlArgs (param: HockeyAppUploadParams) =
    seq {
        yield (String.Format("-sL -w \"{0}%{{http_code}}{0}\"", Regex.Escape(nl)))

        if not (String.IsNullOrEmpty param.VersionId) then
            yield "-X PUT"
        ...
        if not (String.IsNullOrEmpty param.Dsym) then
            yield sprintf "-F \"dsym=@%s\"" param.Dsym
        ...
        if not (String.IsNullOrEmpty param.OwnerId) then
            yield sprintf "-F \"owner_id=%s\"" param.OwnerId

        if not (String.IsNullOrEmpty param.CommitSHA) then
            yield sprintf "-F \"commit_sha=%s\"" param.CommitSHA

        if not (String.IsNullOrEmpty param.BuildServerUrl) then
            yield sprintf "-F \"build_server_url=%s\"" param.BuildServerUrl

        if not (String.IsNullOrEmpty param.RepositoryUrl) then
            yield sprintf "-F \"repository_url=%s\"" param.RepositoryUrl
        ...
        if not (String.IsNullOrEmpty param.AppId) && (String.IsNullOrEmpty param.VersionId) then
            ...
            not (String.IsNullOrEmpty param.AppId)
            && not (String.IsNullOrEmpty param.VersionId)
        then
...
/// [omit]
let private toVersionCurlArgs (param: HockeyAppVersionParams) =
    seq {
        yield (String.Format("-sL -w \"{0}%{{http_code}}{0}\"", Regex.Escape(nl)))
        yield "-X POST"
...
/// [omit]
let private processHockeyAppCmd<'TParam, 'TResponse when 'TParam :> BaseHockeyAppParams>
    defaults
    (setParams: 'TParam -> 'TParam)
    (validateParam: 'TParam -> 'TParam)
    (toCurlArgs: 'TParam -> seq<string>)
    =
    let p = defaults |> setParams |> validateParam
    ...
    |> fun response ->
        let error =
            sprintf
                "Error while posting to HockeyApp.%sMessages: %s%sErrors: %s%s"
                nl
                (String.concat "; " response.Messages)
                nl
                (String.concat "; " response.Errors)
                nl
...
[<System.Obsolete("Use Fake.Api.HockeyApp instead (open Fake.Api and use 'HockeyApp.')")>]
let HockeyApp setParams =
    processHockeyAppCmd<HockeyAppUploadParams, HockeyResponse>
...
[<System.Obsolete("Use Fake.Api.HockeyApp instead (open Fake.Api and use 'HockeyApp.')")>]
let HockeyAppVersion setParams =
    processHockeyAppCmd<HockeyAppVersionParams, HockeyVersionResponse>
