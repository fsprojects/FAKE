...
[<System.Obsolete("Use Fake.DotNet.NuGet.Version instead")>]
type NuGetSearchItemResult = { Id: string; Version: string; Published: DateTime }
...
let IncPatch: NuGetVersionIncrement =
    fun (v: SemVerInfo) -> { v with Build = ""; Patch = (v.Patch + 1) }

/// Increment minor version
[<System.Obsolete("Use Fake.DotNet.NuGet.Version instead")>]
let IncMinor: NuGetVersionIncrement =
    fun (v: SemVerInfo) -> { v with Build = ""; Patch = 0; Minor = (v.Minor + 1) }
...
let getLastNuGetVersion server (packageName: string) =
    ...
    let url =
        sprintf "%s/Search()?$filter=IsLatestVersion&searchTerm='%s'&includePrerelease=false" server packageName
    ...
    let version =
        if
            hasContentType
            && client.ResponseHeaders.Item("Content-Type").Contains "application/json"
        then
            ...
            |> Seq.filter (fun entry ->
                ...
                |> Seq.exists (fun t -> t.Attribute(XName.Get "type").Value = "text" && t.Value = packageName))
            |> Seq.tryHead
            |> function
                | Some e ->
