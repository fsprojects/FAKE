...
let GetPackageVersion deploymentsDir package =
    try
        ...
        version
    with exn ->
        new Exception("Could not detect package version for " + package, exn) |> raise
...
let private replaceAccessKeys parameters (text: string) =
    let replaceKey key (str: string) =
        if isNullOrEmpty key then
            ...
            str.Replace(key, "PRIVATEKEY")

    text
    |> (replaceKey parameters.AccessKey >> replaceKey parameters.SymbolAccessKey)
...
let private pack parameters nuspecFile =
    ...
    let execute args =
        ...
        if result.ExitCode <> 0 || result.Errors.Count > 0 then
            failwithf
                "Error during NuGet package creation. %s %s\r\n%s"
                parameters.ToolPath
                args
                (toLines result.Errors)
...
let rec private publish parameters =
    ...
    let source =
        match parameters.PublishUrl |> normalize, parameters.SymbolPublishUrl |> normalize with
        | None, _ -> ""
        | Some source, None -> sprintf "-source %s" source
        | Some source, Some symSource ->
            sprintf "-source %s -SymbolSource %s -SymbolApiKey %s" source symSource parameters.SymbolAccessKey
    ...
    tracefn
        ...
        parameters.ToolPath
        (replaceAccessKeys parameters args)
...
let getNuspecProperties (nuspec: string) =
    ...
    let getValue name =
        let getWith ns =
            try
                doc |> XPathValue (sprintf "%s:metadata/%s:%s" ns ns name) namespaces |> Some
            with exn ->
                None
...
let getDependencies (packagesFile: string) =
    ...
    [ for package in doc.Descendants(xname "package") -> attribute "id" package, attribute "version" package ]
