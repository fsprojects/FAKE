...
let private createFilters param =
    if isNullOrEmpty param.Filters then
        FilterSet.Empty
    else
        FilterUtils.ParseTestFilterSet param.Filters
...
let private createProject param package =
    ...
    let project =
        TestProject(TestRunnerFactoryName = runnerFactoryName, TestPackage = package)
...
let private createRuntimeSetup param =
    let rtSetup =
        RuntimeSetup(
            RuntimePath =
                Path.GetDirectoryName(AssemblyUtils.GetFriendlyAssemblyLocation(typeof<GallioParams>.Assembly))
        )
