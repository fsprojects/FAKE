...
type FileSetBuilder() =
    ...
    member x.AddMany(fs, patterns: string list) =
        { fs with Includes = fs.Includes @ patterns }
    ...
    member x.ExcludeMany(fs: FileIncludes, patterns: string list) =
        { fs with Excludes = fs.Excludes @ patterns }
