...
let private bindApplicationPool (appPool: ApplicationPool) (app: Application) = app.ApplicationPoolName <- appPool.Name
...
let RemoveBindingFromSite bindingInformation bindingProtocol (siteName: string) (manager: ServerManager option) =
    doWithManager
        (fun m ->
            ...
            match
                site.Bindings
                |> Seq.tryFind (fun b -> b.Protocol = bindingProtocol && b.BindingInformation = bindingInformation)
            with
...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
let AddBindingToSite
    (bindingInformation: string)
    (bindingProtocol: string)
    (siteName: string)
    (manager: ServerManager option)
    =
    doWithManager
        (fun m ->
            let site = m.Sites.[siteName]
            ...
            match
                site.Bindings
                |> Seq.exists (fun b -> b.Protocol = bindingProtocol && b.BindingInformation = bindingInformation)
            with
...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
type SiteConfig(name: string, binding: string, physicalPath: string, appPool: string, ?id: int64, ?protocol: string) =
    class
...
[<System.Obsolete("This API is obsolete. There is no alternative in FAKE 5 yet. You can help by porting this module.")>]
type ApplicationPoolConfig
    (
        name: string,
        ?runtime: string,
        ?allow32on64: bool,
        ?identity: ProcessModelIdentityType,
    ...
    class
        ...
        member this.identity =
            defaultArg identity ProcessModelIdentityType.ApplicationPoolIdentity
...
let ApplicationPool (config: ApplicationPoolConfig) (mgr: ServerManager) =
    ...
    | _ -> MergeAppPoolProperties appPool config
...
let AppCmd (command: string) =
    ...
    then
        failwithf "AppCmd.exe %s failed." command
