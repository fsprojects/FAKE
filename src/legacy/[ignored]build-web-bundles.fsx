...
Target "Clean" (fun _ -> CleanDirs [ buildDir; targetDir ])
...
Target "ZipBundles" (fun _ ->
    for dir in IO.Directory.EnumerateDirectories(buildDir) do
        ...
        let files =
            IO.Directory.EnumerateFiles(dir.FullName, "*.*", IO.SearchOption.AllDirectories)
...
// Dependencies
"Clean" =?> ("SetAssemblyInfo", not isLocalBuild)
==> "BuildBundles"
==> "ZipBundles"
==> "Default"
