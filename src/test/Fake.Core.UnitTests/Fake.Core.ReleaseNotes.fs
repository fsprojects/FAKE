...
let tests = 
  testList "Fake.Core.ReleaseNotes.Tests" [
    ...
    // https://semver.org
    testList "SemVer" [ 
      testCase "Test that we can parse basic SemVer." <| fun _ ->
        let releaseNotesLines = [
          "## 5.15.3 - 2019-07-03"
        ]
        let releaseNotes = ReleaseNotes.parse releaseNotesLines
      ...
      testCase "Test that we can parse SemVer with pre release." <| fun _ ->
        ...
        checkPreRelease releaseNotesLines_case4 "x.7.z.92"
      testCase "Test that we can parse SemVer with build metadata." <| fun _ ->
      ...
      testCase "Test that we can parse SemVer tests (link in comment)." <| fun _ ->
          let releaseNotesLines_case1 = [ "## 1.0.0-alpha-a.b-c-somethinglong+build.1-aef.1-its-okay - 2019-07-03" ] 
          let releaseNotesLines_case2 = [ "## 1.0.0----RC-SNAPSHOT.12.9.1--.12+788 - 2019-07-03" ] 
          let releaseNotesLines_case3 = [ "## 1.0.0----R-S.12.9.1--.12+meta - 2019-07-03" ] 
          let releaseNotesLines_case4 = [ "## 1.0.0+0.build.1-rc.10000aaa-kk-0.1 - 2019-07-03" ] 
          ...
          let checkPreRelease (rn: string list) (preReleaseName: string option) (metadata:string option) =
              ...
              if preReleaseName.IsSome then
                Expect.isTrue semVer.PreRelease.IsSome $"{rn.[0]} PreRelease.IsSome"
                Expect.equal semVer.PreRelease.Value.Origin preReleaseName.Value $"{rn.[0]} PreRelease.Value.Origin"
              if metadata.IsSome then
                Expect.equal semVer.BuildMetaData metadata.Value $"{rn.[0]} BuildMetaData"
          checkPreRelease releaseNotesLines_case1 (Some "alpha-a.b-c-somethinglong") (Some "build.1-aef.1-its-okay")
