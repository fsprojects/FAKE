...
let tests =
    ...
    testList
        "Fake.DotNet.Fsi.Tests"
        [ testCase "Test that default params have no defines"
          <| fun _ ->
              let cmdList = Fsi.FsiParams.Create() |> Fsi.FsiParams.ToArgsList

              Expect.isFalse
                  (cmdList |> List.exists isDefine)
                  "FsiParams.Create() |> FsiParams.ToArgsList should not specify -d"

          testCase "Test that Define alone adds one -d flag"
          <| fun _ ->
              let cmdList =
                  { Fsi.FsiParams.Create() with Define = "DEBUG" } |> Fsi.FsiParams.ToArgsList
          ...
          <| fun _ ->
              ...
              Expect.contains
                  cmdList
                  "-d:DEBUG"
                  "Definitions = [\"DEBUG\"; \"GUBED\"] should create the -d:DEBUG parameter"

              Expect.contains
                  cmdList
                  "-d:GUBED"
                  "Definitions = [\"DEBUG\"; \"GUBED\"] should create the -d:GUBED parameter"

              Expect.hasCountOf cmdList 2u isDefine "Definitions should create both -d parameters"

          testCase "Test that Definitions can be used together with Define"
          <| fun _ ->
              let cmdList =
                  { Fsi.FsiParams.Create() with
                      Definitions = [ "DEBUG"; "GUBED" ]
