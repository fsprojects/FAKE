...
let tests =
    testList
        "Fake.Testing.ReportGenerator.Tests"
        [ testCase "Test that new argument generation  with default parameters"
          <| fun _ ->
              let expectedPath, commandLine = runCreateProcess id

              Expect.equal
                  commandLine
                  (sprintf
                      "%s -reports:report1.xml;report2.xml -targetdir:targetDir -reporttypes:Html -verbosity:Verbose"
                      expectedPath)
                  "expected proper command line"

          testCase "Test that new argument generation with all parameters"
          <| fun _ ->
              let expectedPath, commandLine =
                  runCreateProcess (fun p ->
                      { p with
                          ReportTypes = [ ReportGenerator.ReportType.Html; ReportGenerator.ReportType.MHtml ]
              ...
              Expect.equal
                  commandLine
                  (sprintf
                      "%s -reports:report1.xml;report2.xml -targetdir:targetDir -reporttypes:Html;MHtml -sourcedirs:source1;source2 -historydir:history -assemblyfilters:+a1*;-a2* -classfilters:+c1*;-c2* -filefilters:+f1*;-f2* -tag:mytag -verbosity:Verbose"
                      expectedPath)
                  "expected proper command line"
          ...
          <| fun _ ->
              ...
              Expect.equal
                  commandLine
                  (sprintf
                      "%s -reports:report1.xml;report2.xml -targetdir:targetDir -reporttypes:Cobertura -verbosity:Verbose"
                      expectedPath)
                  "expected proper command line"

          testCase "Test that full Framework works"
          <| fun _ ->
              ...
              Expect.equal
                  (RawCommand(file, args)).CommandLine
                  (sprintf
                      "%s -reports:report1.xml;report2.xml -targetdir:targetDir -reporttypes:Html -verbosity:Verbose"
                      path)
                  "expected proper command line"

          testCase "Test that global tool works"
          <| fun _ ->
          ...
          testCase "Test that local tool override works"
          <| fun _ ->
              let cp =
                  rawCreateProcess (fun p ->
                      { p with
                          ToolType = ToolType.CreateLocalTool() |> ToolType.withDefaultToolCommandName "alternative" })
          ...
          testCase "Test that local tool works"
          <| fun _ ->
              let cp =
                  rawCreateProcess (fun p -> { p with ToolType = ToolType.CreateLocalTool() })
              ...
              Expect.equal
                  (RawCommand(file, args)).CommandLine
                  (sprintf
                      "reportgenerator -reports:report1.xml;report2.xml -targetdir:targetDir -reporttypes:Html -verbosity:Verbose")
                  "expected proper command line"

          testCase "Test that DotNet can override dotnet"
          <| fun _ ->
              let cp =
                  rawCreateProcess (fun p ->
                      { p with
                          ToolType =
                              ...
                              |> ToolType.withDotNetOptions (fun o ->
                                  { o with DotNetCliPath = "some/dotnet/path/dotnet.exe" }) })
