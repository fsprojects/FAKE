...
let getProps options =
    let args = MSBuild.CliArguments.Create() |> Coverlet.withMSBuildArguments options
    args.Properties

[<Tests>]
let tests =
    testList
        "Fake.DotNet.Testing.Coverlet.Tests"
        [ testCase "Test that full properties are converted"
          <| fun _ ->
              let props =
                  getProps (fun p ->
          ...
          testCase "Test that default properties are converted"
          <| fun _ ->
