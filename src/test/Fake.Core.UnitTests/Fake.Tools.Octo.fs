...
let defaultTests =
    testList
        "Fake.Tools.Octo Tests"
        [ testCase "List Environments"
          <| fun _ ->
              let expected = [ "list-environments" ]
              let actual = Fake.Tools.Octo.Command.ListEnvironments |> Fake.Tools.Octo.commandLine
          ...
          testCase "Create Release Default"
          <| fun _ ->
              let expectedCommand = [ "create-release" ]
          ...
          testCase "Push Command"
          <| fun _ ->
              let expect =
                  [ "push"; "--package=Package-1 --package=Package-2"; "--replace-existing" ]
              ...
              let actual =
                  pushOption |> Fake.Tools.Octo.Command.Push |> Fake.Tools.Octo.commandLine

              Expect.hasLength actual expect.Length ""
              Expect.sequenceEqual actual expect ""

          testCase "Create Release Fully Filled Out"
          <| fun _ ->
              let expectedCommand =
                  [ "create-release"
                    "--project=Project-1"
          ...
          testCase "Create Release With Common"
          <| fun _ ->
              let expectedCommand =
                  [ "create-release"
                    "--project=Project-1"
          ...
          testCase "Create Deploy Full Filled Out"
          <| fun _ ->
              let expected =
                  [ "deploy-release"
                    "--project=Project-1"
          ...
          testCase "Delete Release fully Fulled Out"
          <| fun _ ->
              let expectedCommand =
                  [ "delete-releases"
                    "--project=Project-1"
          ...
          testCase "Realistic Octo Args To Windows Command Line"
          <| fun _ ->
              ...
              let setReleaseParams =
                  (fun (ro: Fake.Tools.Octo.CreateReleaseOptions) ->
                      { ro with
                          Project = "MyProject"
                          Version = "1234567890.12.34"
                          Packages = [ "MyPackage:1234567890.12.34" ]
                          Common = setCommon ro.Common })

              let setDeployParams =
                  (fun (dOpt: Fake.Tools.Octo.DeployReleaseOptions) ->
                      { dOpt with
                          Project = "MyProject"
                          Version = "1234567890.12.34"
                          DeployTo = "MyEnvironment"
                          Common = setCommon dOpt.Common
                          Progress = true }
                      |> Some)

              let command =
                  (Fake.Tools.Octo.CreateRelease(
                      (setReleaseParams Fake.Tools.Octo.releaseOptions),
                      (setDeployParams Fake.Tools.Octo.deployOptions)
                  ))
              ...
              let args =
                  (List.append
                      (Fake.Tools.Octo.commandLine command)
                      (Fake.Tools.Octo.serverCommandLine releaseOptions.Common.Server))
                  |> Fake.Core.Arguments.OfArgs

              let command = args.ToWindowsCommandLine

              Expect.equal
                  command
                  "create-release --project=MyProject --version=1234567890.12.34 --package=MyPackage:1234567890.12.34 --project=MyProject --deployto=MyEnvironment --version=1234567890.12.34 --progress --server=https://myoctopus-server.com --apikey=octoApiKey"
                  "The output should be runnable on windows." ]
