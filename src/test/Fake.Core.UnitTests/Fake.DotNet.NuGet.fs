...
let tests =
    testList
        "Fake.DotNet.NuGet.Tests"
        [ testCase "Test that the default nuget push arguments returns empty string"
          <| fun _ ->
              let cli =
          ...
          testCase "Test that the nuget push arguments with all params set returns correct string"
          <| fun _ ->
              let param =
              ...
              let cli = param |> NuGet.toPushCliArgs |> Args.toWindowsCommandLine

              let expected =
                  "-ApiKey abc123 -DisableBuffering -NoSymbols -NoServiceEndpoint -Source MyNuGetSource -SymbolApiKey MySymbolApiKey -SymbolSource MySymbolSource -Timeout 360"

              Expect.equal cli expected "Push args generated correctly."

          test "Incrementing Patch for a SemVerInfo" {
              Expect.equal
                  (SemVer.parse ("1.1.0") |> IncPatch |> string)
                  "1.1.1"
                  "Incremented Patch from 1.1.0 should be 1.1.1"
          }

          test "Incrementing Minor for a SemVerInfo" {
              Expect.equal
                  (SemVer.parse ("1.1.1") |> IncMinor |> string)
                  "1.2.0"
                  "Incremented Minor from 1.1.1 should be 1.2.0"
          }

          test "Incrementing Major for a SemVerInfo" {
              Expect.equal
                  (SemVer.parse ("1.1.1") |> IncMajor |> string)
                  "2.0.0"
                  "Incremented Patch from 1.1.1 should be 2.0.0"
          }
          ...
          testCase "Getting latest version of FAKE from NuGet feed"
          <| fun _ ->
              let package: NuGet.NugetPackageInfo =
                  NuGet.getLatestPackage (NuGet.getRepoUrl ()) "FAKE"

              Expect.isTrue package.IsLatestVersion "getting latest version of a package"

          testCase "Getting latest version of FAKE and package info are populated"
          <| fun _ ->
              let package: NuGet.NugetPackageInfo =
                  NuGet.getLatestPackage (NuGet.getRepoUrl ()) "FAKE"
          ...
          testCase "Requesting specific version of FAKE"
          <| fun _ ->
              let package: NuGet.NugetPackageInfo =
                  NuGet.getPackage (NuGet.getRepoUrl ()) "FAKE" "1.46.2"

              Expect.isFalse package.IsLatestVersion "getting version 1.46.2 of FAKE"

          testCase "Requesting un-registered version of FAKE throws exception"
          <| fun _ ->
              Expect.throws
                  (fun _ -> NuGet.getPackage (NuGet.getRepoUrl ()) "FAKE" "-1.55.0" |> ignore)
                  "Version -1.55.0 is not registered for FAKE!"

          testCase "Requesting specific version of FAKE and package info are populated"
          <| fun _ ->
              let package: NuGet.NugetPackageInfo =
                  NuGet.getPackage (NuGet.getRepoUrl ()) "FAKE" "1.46.2"
          ...
          testCase "Search by title returns results for matching packages by provided title"
          <| fun _ ->
              let package =
                  NuGet.searchByTitle (NuGet.getRepoUrl ()) "FAKE"
