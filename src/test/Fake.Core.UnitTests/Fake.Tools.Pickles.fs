...
let tests =
    testList
        "Fake.Tools.Pickles.Tests"
        [ testCase "Test that new argument generation works with minimal parameters"
          <| fun _ ->
              let expectedPath, commandLine = runCreateProcess id

              Expect.equal commandLine (sprintf "%s --df dhtml" expectedPath) "expected proper command line"

          testCase "Test that new argument generation works with all parameters"
          <| fun _ ->
              ...
              Expect.equal
                  commandLine
                  (sprintf
                      "%s -f features -o output --sn sut --sv sutver --l de --df dhtml --trfmt xunit2 --lr TestResult1.xml;TestResult2.xml --exp --enableComments=false --et et1;et2 --ht ht1;ht2"
                      expectedPath)
