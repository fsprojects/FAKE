    ...
    if Environment.isWindows then
        [ testCase "Test that default arguments are processed as expected"
          <| fun _ ->
              ...
              let pwd = Shell.pwd ()
              Expect.isTrue (p.IncludeSummaryReport) "A field should have non-default value for a bool"
          ...
          testCase "Test that default arguments are processed as expected (containing spaces)"
          <| fun _ ->
              let p = FxCop.Params.Create()
          ...
          testCase "Test that all arguments are processed as expected"
          <| fun _ ->
              let p =
                  { FxCop.Params.Create() with
                      DependencyDirectories = [ Guid.NewGuid().ToString(); Guid.NewGuid().ToString() ]
                      ImportFiles = [ Guid.NewGuid().ToString(); Guid.NewGuid().ToString() ]
                      RuleLibraries = [ Guid.NewGuid().ToString(); Guid.NewGuid().ToString() ]
                      Rules = [ Guid.NewGuid().ToString(); Guid.NewGuid().ToString() ]
                      CustomRuleset = Guid.NewGuid().ToString()
                      ConsoleXslFileName = Guid.NewGuid().ToString()
                      ReportFileName = Guid.NewGuid().ToString()
                      OutputXslFileName = Guid.NewGuid().ToString()
                      PlatformDirectory = Guid.NewGuid().ToString()
                      ProjectFile = Guid.NewGuid().ToString()
                      Types = [ Guid.NewGuid().ToString(); Guid.NewGuid().ToString() ]
                      WorkingDirectory = Guid.NewGuid().ToString()
                      CustomDictionary = Guid.NewGuid().ToString()
                      ApplyOutXsl = true
          ...
          testCase "Test that generated code should be ignored"
          <| fun _ ->
          ...
          <| fun _ ->
              ...
              Expect.equal
                  ...
                  [ "/f:" + dummy + ""
                    ...
                    "/s"
                    "/v" ]
                  "No output to console expected"
          testCase "Test that summary reporting can be switched off"
          <| fun _ ->
          ...
          <| fun _ ->
              ...
              Expect.equal
                  ...
                  [ "/c"
                    ...
                    "/u"
                    "/v" ]
                  "results should be in project file"
          testCase "Test that output can be forced"
          <| fun _ ->
              ...
              Expect.equal
                  ...
                  [ "/c"
                    ...
                    "/s"
                    "/v" ]
                  "Output should be forced"
          testCase "Test that Xsl is defaulted"
          <| fun _ ->
          ...
          <| fun _ ->
              ...
              Expect.equal proc.CommandLine (dummy + " " + String.Join(" ", args)) "tool should match"
              Expect.equal proc.WorkingDirectory (Some <| Shell.pwd ()) "WorkingDirectory should default"
          ...
          <| fun _ ->
              ...
              let proc = FxCop.createProcess p args
              Expect.equal proc.CommandLine (dummy + " " + String.Join(" ", args)) "tool should match"
              Expect.equal proc.WorkingDirectory (Some dummy2) "WorkingDirectory should match input"
          ...
          <| fun _ ->
              ...
              let expected =
                  sprintf """%s /c /f:%s /o:%s /s /v""" dummy (Seq.head assemblies) dummy2

              Expect.equal proc.CommandLine expected "composed command line should match"
              Expect.equal proc.WorkingDirectory (Some <| Shell.pwd ()) "WorkingDirectory should default"
          testCase "Test errors are read as expected"
          <| fun _ ->
              let dummy = Guid.NewGuid().ToString()
              ...
              lock serializingObject (fun _ ->
                  ...
                  try
                      ...
                      Expect.equal (FxCop.checkForErrors dummy) (23, 42, 17, 5) "Results should match"
                  finally
                  ...
                  (fun () ->
                      lock serializingObject (fun _ ->
                          ...
                          try
                              ...
                              FxCop.XmlReadInt <- saved))
                  (fun ex -> Expect.equal ex.Message "FxCop test failed." "should have simple failure")
          testCase "Analysis failure is handled"
          <| fun _ ->
              ...
              let messages =
                  [| "FxCop found 1 critical errors."
                     "FxCop found 1 errors."
                     "FxCop found 1 critical warnings."
                     "FxCop found 1 warnings."
                     String.Empty |]
              ...
              |> Seq.iter (fun (level, mapIndex) ->
                  let p =
                      { FxCop.Params.Create() with
                          ReportFileName = dummy
                          FailOnError = enum<FxCop.ErrorLevel> level }

                  let result = { ExitCode = 0; Result = () }
                  ...
                  if level >= int FxCop.ErrorLevel.CriticalError && level >= mapIndex + 2 then
                      Expect.throwsC op (fun ex ->
                          Expect.equal
                              ex.Message
                              messages.[mapIndex]
                              (sprintf "should have expected failure level=%d index=%d" level mapIndex))
                  else
                      try
                          op ()
                      with x ->
                          printfn "Unexpected failure %A level=%d index=%d" x level mapIndex
                          reraise ()) ]
    else
