...
let tests =
    testList
        "Fake.DotNet.Testing.NUnit.Tests"
        [ testCase "Test that we write and delete arguments file"
          <| fun _ ->
              let cp =
                  NUnit3.createProcess
                      Path.GetTempFileName
                      (fun param -> { param with ToolPath = "mynunit.exe" })
                      [| "assembly.dll" |]
          ...
          testCase "Test that we support file-paths with space - #2180"
          <| fun _ ->
              let d = Directory.CreateDirectory "some path"

              let cp =
                  NUnit3.createProcess
                      (fun _ -> "some path/with spaces.txt")
                      (fun param -> { param with ToolPath = "mynunit.exe" })
                      [| "assembly.dll" |]
              ...
              Expect.equal
                  (args |> Arguments.toArray).[0]
                  "@some path/with spaces.txt"
                  "expected to support file paths with spaces"
