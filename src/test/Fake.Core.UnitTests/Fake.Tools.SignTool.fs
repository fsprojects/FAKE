...
let tests =
    testList "Fake.Tools.SignTool.Tests" [
        ...
        testProperty "time stamp options" <| fun (timestampOptions: SignTool.TimeStampOptions) ->
            ...
            Expect.isFalse (actualSigntoolArgs.EndsWith(' ')) "Expected arguments to not end with a space"
            let serverUrl = timestampOptions.TimeStamp.ServerUrl |> Option.defaultValue "http://timestamp.digicert.com"
            actualSigntoolArgs
            ...
            |> expectIfEnum (sprintf "/t \"%s\"" serverUrl) timestampOptions.TimeStamp.Algorithm SignTool.DigestAlgorithm.SHA1 "TimeStamp.Algorithm.SHA1"
            |> expectIfEnum (sprintf "/tr \"%s\" /td \"sha256\"" serverUrl) timestampOptions.TimeStamp.Algorithm SignTool.DigestAlgorithm.SHA256 "TimeStamp.Algorithm.SHA256"
