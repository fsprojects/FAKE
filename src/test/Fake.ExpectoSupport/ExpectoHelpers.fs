...
module ExpectoHelpers =
    ...
    let literateConsoleTarget = LiterateConsoleTarget([|"ExpectoFake"|], ExpectoConfig.defaultConfig.verbosity)
      ...
      { beforeRun = fun _tests ->
        ...
        summary = fun _config summary ->
          ...
          printfn "EXPECTO! %s tests run in %O for %s – %s passed, %s ignored, %s failed, %s errored. %s"
            (List.sumBy (fun (_,r: TestSummary) -> if r.result.isIgnored then 0 else r.count) summary.results |> commaString)
            summary.duration
            commonAncestor
