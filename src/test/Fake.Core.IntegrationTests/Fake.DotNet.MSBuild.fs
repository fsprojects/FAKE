...
let simplePropertyTest propValue =
    ...
    let setMSBuildParams (defaults: MSBuild.CliArguments) =
        { defaults with
            ...
            Targets = [ "Test" ]
            NoLogo = true
            Properties = [ "Property1", propValue ] }

    let setParams (p: DotNet.MSBuildOptions) =
        p.WithMSBuildParams setMSBuildParams |> dotnetWorkingDir dllPath
    ...
    | Choice2Of2 (e, result) ->
        ...
        raise
        <| exn (sprintf "simplePropertyTest failed, msbuild output was: \n%s\n" lines, e)
...
let simplPathTest pathCased propValue =
    ...
    | Choice1Of2 result ->
        ...
        if Environment.isWindows || pathCased = "PATH" then
            Expect.stringContains
                lines
                (sprintf "$PATH: '%s'" (expected propValue))
                "Expected to find property value in msbuild output"
        else
...
let tests =
    testList
        "Fake.DotNet.MSBuild.IntegrationTests"
        [ Process.setEnableProcessTracing true
          let prefixPath =
              if Environment.isWindows then
                  ...
                  | Some shPath -> System.IO.Path.GetDirectoryName shPath + string System.IO.Path.PathSeparator
          ...
          yield
              ...
              <| fun _ ->
                  let value =
                      "Data Source=xxx,1433;Initial Catalog=xxx;User Id=xxx;Password=xxx;Integrated Security=False;Persist Security Info=True;Connect Timeout=30;Encrypt=True;MultipleActiveResultSets=True"
