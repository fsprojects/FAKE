...
let tests =
    testList
        "Fake.IO.File.IntegrationTests"
        [ testCase "File.getVersion throws InvalidOperationException #2378"
          <| fun _ ->
              if Environment.isWindows then // On non-windows the API returns managed assembly info, see https://github.com/dotnet/corefx/blob/5fb98a118bb19a91e8ffb5c17ff5e7c00a4c05ee/src/System.Diagnostics.FileVersionInfo/src/System/Diagnostics/FileVersionInfo.Unix.cs#L20-L28
                  ...
                  Expect.throwsT<System.InvalidOperationException>
                      (fun () -> File.getVersion testFile |> ignore<string>)
                      "Expected InvalidOperationException for missing file version"
          testCase "File.tryGetVersion works when component is missing #2378"
          <| fun _ ->
              if Environment.isWindows then // On non-windows the API returns managed assembly info, see https://github.com/dotnet/corefx/blob/5fb98a118bb19a91e8ffb5c17ff5e7c00a4c05ee/src/System.Diagnostics.FileVersionInfo/src/System/Diagnostics/FileVersionInfo.Unix.cs#L20-L28
                  let testFile = getTestFile "NoVersionTestFile.dll"
                  Expect.equal (File.tryGetVersion testFile) None "Expected None for missing file version"

          testCase "Files created using File.create can be used immediately - #2183"
          <| fun _ ->
          ...
          testCase "Shell.mv works as expected when src is a file and dst doesnt exist - #2293"
          <| fun _ ->
              use d = createTestDir ()
          ...
          testCase "Shell.mv works as expected when src is a file and dst is a file - #2293"
          <| fun _ ->
              use d = createTestDir ()
              let f1 = Path.Combine(d.Dir, "1")
              let f2 = Path.Combine(d.Dir, "2")
              ...
              Expect.isTrue (File.Exists f2) "f2 should exist"
              Expect.equal (File.ReadAllText(f2)) "test" "Unexpected content in test file"
          testCase "Shell.mv works as expected when src is a file dst is a dir - #2293"
          <| fun _ ->
              use d = createTestDir ()
              let f1 = Path.Combine(d.Dir, "1")
              ...
              Expect.isTrue (File.Exists f2) "f2 should exist"
              Expect.equal (File.ReadAllText(f2)) "test" "Unexpected content in test file"
          testCase "Shell.mv works as expected when src is a dir and dst doesnt exist - #2293"
          <| fun _ ->
              use d = createTestDir ()
              ...
              Expect.isTrue (File.Exists f2) "f2 should exist"
              Expect.equal (File.ReadAllText(f2)) "test" "Unexpected content in test file"
          testCase "Shell.mv throws as expected when src is a dir and dst is a file - #2293"
          <| fun _ ->
              use d = createTestDir ()
              let d1 = Path.Combine(d.Dir, "1")
              let f2 = Path.Combine(d.Dir, "2")
          ...
          testCase "Shell.mv works as expected when src is a dir dst is a dir - #2293"
          <| fun _ ->
              use d = createTestDir ()
              let d1 = Path.Combine(d.Dir, "1")
              let d2 = Path.Combine(d.Dir, "2")
