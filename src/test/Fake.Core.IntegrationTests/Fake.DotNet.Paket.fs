...
let tests =
    testList
        "Fake.DotNet.Paket.IntegrationTests"
        [ testCase "findPaketExecutable works for global tools - #2361"
          <| fun _ ->
              ...
              let paketExe =
                  Path.Combine(paketDir, (if Environment.isWindows then "paket.exe" else "paket"))
