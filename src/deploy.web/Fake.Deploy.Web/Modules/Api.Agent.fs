...
type AgentResponse<'t> = { case: string; values: 't[] }

type ApiAgent (dataProvider : IDataProvider) as http =
    inherit FakeModule("/api/v1/agent")

    let logger = LogManager.GetLogger(http.GetType().Name)

    let getAgent agentId =
        dataProvider.GetAgents [agentId] |> Seq.tryFind(fun i -> true)

    let callAgent (agentId:string) (urlPart:string) (f: string -> 'T) logError =
        http.returnAsJson 
            (fun () ->
    ...
    do            
        ...
        http.get "/details/{agentId}" (fun p -> 
            let agentId = p ?> "agentId"
            callAgent
                agentId "statistics" fromJSON<MachineDetails>
        ...
        http.get "/details/{agentId}/{status}" (fun p ->
            let agentId = p ?> "agentId"
            let status = p ?> "status"
            callAgent
                agentId (sprintf "deployments?status=%s" status) fromJSON<AgentResponse<NuSpecPackage[]>>
                (fun e -> logger.Error(sprintf "An error occured retrieving details for agent %s (status = %s)" agentId status,e))
        )
